...
- [ ] **Action:** on a clean machine with Node ≥ 16:
      ...
      activate skills in the current project (default: every skill in the corpus, computed at
      runtime; `--solo` 8 / `--grow` 18 / `--skill <name>` / `--status`).
...
maintainer session (marketplace add + `zeroes-ones-all` + flagship 30); item 4's
