...
- Our skills already follow the open `SKILL.md` standard (frontmatter name/description, optional
  ...
  ship.
- **Implemented & verified against Claude Code 2.1.222 (2026-09-07):**
  `.claude-plugin/marketplace.json` (generated by `scripts/emit-marketplace.py`, stdlib,
  ...
  model does **not** validate in current Claude Code (`claude plugin validate` rejects it),
  which is why the schema was corrected.
- Still open: the separate **private "pro" marketplace** repo for the paid tier.

**Packaging shape (as implemented):**
1. `.claude-plugin/marketplace.json` at the repo root — 38 plugins, each `source` a
   relative dir that exists with `plugin.json` + a `skills` symlink resolving to skill dirs
   (validated at generation time).
2. `plugins/<name>/plugin.json` — `{name, version, description, author, skills: ["./skills"]}`;
   `plugins/<name>/skills` → symlink (nothing duplicated).
3. `scripts/emit-marketplace.py` — regenerate on any domain/skill change; rerun output is
   byte-identical (idempotent).
4. Validate every PR: `claude plugin validate .claude-plugin/marketplace.json` (verified:
   passes with 0 errors on the generated manifest); JSON well-formedness also enforced by the
   generator.
5. Publish a second, **private "pro" marketplace** repo for enterprise/org features (paid tier).
