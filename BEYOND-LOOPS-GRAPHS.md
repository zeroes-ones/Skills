...
**Build:** `skill-retrieval` layer: embed every skill **body** (research: body > metadata) +
...
**Why:** static routers stop scaling; hundreds of skills today, thousands tomorrow; accuracy degrades when
...
|-------|-------|--------|----------------|
| **P0 (now, low risk)** | B4 exporter + B2 golden-set scaffolding + B6 index skeleton | run-state, `evals/`, chain graph, audit tooling | export-trace runs on both shipped checkpoints; ≥3 golden case files; index builds over the whole corpus |
...
2. **Vectors — yes, next after P1.** Rationale: hundreds of skills today, thousands tomorrow; the static
