...
8. **Coordination-aware** — Every skill knows which other skills it must coordinate with, when to escalate, and what to communicate at each scale. Chain edges are symmetric: if A feeds_into B, then B consumes_from A.
...
### The Neighbor Test
Reading this skill tells you exactly which skill to use next, what that skill needs from you, and what it produces. Cross-skill integration examples show actual command sequences. The YAML `chain:` block is symmetric — if this skill `feeds_into` another, that skill's `consumes_from` includes this one. Run `python3 scripts/validate_chains.py` to verify 0 asymmetries.
...
### Structure
- [ ] YAML frontmatter with ALL required fields including token_budget, type, version, updated, chain (with symmetric consumes_from/feeds_into)
