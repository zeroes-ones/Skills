...
<!-- STANDARD: 3min -->
Full computation → references/fundamental-analyst-computations.md

### Phase 1: Financial Statement Collection (Full detail → references)
1. Pull 10-K/10-Q from SEC EDGAR. Verify GAAP/IFRS. Collect: income statement (5yr), balance sheet (5yr), cash flow (5yr). Extract key line items.
   |-- Complete when: Statements collected [VERIFIED with SEC source]. Key items extracted. Reporting standard noted.

### Phase 2: Ratio Analysis (Full detail → references)
1. Profitability: gross/operating/net margins, ROE, ROA, ROIC. Liquidity: current/quick ratio. Leverage: debt/equity, interest coverage. Efficiency: asset/inventory turnover. Valuation: P/E, P/B, EV/EBITDA, P/FCF.
   |-- Complete when: All ratios [COMPUTED] against industry peers. Trends (5yr) plotted. Red flags flagged.

### Phase 3: DCF Valuation (Full detail → references)
1. Project FCF (5yr explicit + terminal). WACC computation. Sensitivity: ±1% WACC, ±10% growth. Bull/base/bear.
   |-- Complete when: DCF model built [COMPUTED]. Sensitivity table populated. Margin of safety vs market price [COMPUTED].

### Phase 4: Moat & Competitive Analysis (Full detail → references)
1. Porter's 5 Forces. Moat sources: switching costs, network effects, intangibles, cost advantage, efficient scale. Assess durability.
   |-- Complete when: Moat classified (wide/narrow/none). Competitive threats documented.

### Phase 5: Management & Governance (Full detail → references)
1. Track record: capital allocation, buyback timing, acquisition ROI. Insider ownership/transactions. Compensation alignment.
   |-- Complete when: Management scorecard. Red flags: excessive dilution, empire-building M&A, misaligned incentives.

### Phase 6: Recommendation (Full detail → references)
1. Weight: quantitative 60% (DCF + ratios) / qualitative 40% (moat + management). Generate BUY/HOLD/SELL with price target and conviction score (0-10 multiplicative).
   |-- Complete when: Recommendation generated [ESTIMATED]. Conviction scored. Risk factors listed.
## Decision Trees

<!-- STANDARD: 3min -->
Full detail → references/fundamental-analyst-computations.md

### DT1: Stock Screener → Full detail in references
```
DCF margin of safety >20%? → NO → PASS (too expensive)
  ↓ YES
ROIC > WACC? → NO → PASS (value-destroying)
  ↓ YES
Debt/Equity < industry median? → NO → Flag: high leverage risk
  ↓ YES
Insider buying? → YES → Strong confidence boost. NO → Neutral.
  ↓ → BUY candidate ✓
```

### DT2: Sell Decision → Full detail in references
```
Thesis broken? → YES → SELL regardless of P&L
  ↓ NO
DCF FV < 80% of entry? → YES → Overvalued → SELL
  ↓ NO
