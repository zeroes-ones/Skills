...
| Symptom | Root Cause | Fix | Lesson |
|---------|-----------|-----|--------|
| Cannot determine IV rank for underlying | Data source unavailable, insufficient historical data (less than 1 year), or broker platform limits | First: Check data sources (broker platform, free sources like Market Chameleon or Barchart). Compute manually if IV data is available: `(current_IV − 1yr_min_IV) / (1yr_max_IV − 1yr_min_IV) × 100`. If that fails: Use IV percentile from nearest liquid expiration (30-45 DTE ATM). If 1-year data unavailable, use 3-month range with explicit note that rank is based on shorter lookback. Last resort: Downgrade strategy to "IV-AGNOSTIC" — use defined-risk vertical spreads with delta-based strikes. Debit strategies only if directional conviction is STRONG, credit strategies only at 25-delta short with 2× credit stop. Flag: "Strategy constructed without IV rank — risk assessment degraded." | IV rank is the foundation of every strategy decision. Without it you are trading blind — always have a fallback method for rank calculation before the trade. |
| UOA signal data incomplete (missing OI ratio, multi-leg detection) | Incomplete raw options flow data — provider may not deliver full signal (volume vs OI, multi-leg detection) | First: Contact quantitative-analyst for complete signal data. Check raw options flow data for OI at the strike/expiration. If that fails: Compute OI ratio from available data: `volume / yesterday_oi` as approximation. For multi-leg: check for same-underlying trades in ±60s window with correlated strikes. Last resort: Treat signal as "UNCONFIRMED." Reduce conviction by 2 levels (STRONG→WEAK, MODERATE→IGNORE). Use non-UOA strategy selection (IV rank + technicals only). Do not mirror UOA structure without multi-leg confirmation. | An incomplete UOA signal is worse than no signal — it creates false conviction. Always downgrade conviction when data is partial and never replicate UOA structure without multi-leg confirmation. |
| Strategy selected but bid-ask spread on target strikes exceeds 5% of option price | Illiquid options market — low volume or wide spreads make the strategy untradeable at fair prices | First: Widen strike search — move 1 strike in favorable direction. If still wide, move to next expiration (monthly vs weekly). If that fails: Switch strategy to one with fewer legs (covered call instead of vertical spread on illiquid underlying). Fewer legs = less slippage. Last resort: Skip the underlying entirely. Illiquid options are not tradeable for defined-risk strategies — the bid-ask spread IS your edge, and if it's too wide, you have no edge. Flag: "Underlying rejected — insufficient options liquidity." | Liquidity is not optional. A strategy that can't be entered at fair prices is not a viable strategy — the spread consumes your edge before you even place the order. |
| Account cannot support the recommended strategy due to margin/approval level | Broker approval level, margin requirements, or buying power constraints prevent execution of the optimal strategy | First: Reduce strategy to fit approval level — naked options → vertical spread, portfolio margin → Reg-T margin sizing. Check broker-specific rules. If that fails: Reduce position size to fit within available buying power. Accept lower absolute return for compliance. Last resort: Switch to a capital-efficient alternative — covered call instead of naked put (uses stock as collateral), vertical spread instead of iron condor (fewer legs = lower margin). Flag: "Strategy adapted for account constraints — original strategy required {X} approval level." | Account constraints are a reality, not a failure. The best strategy is the one that fits your account — a perfect strategy you can't execute is worthless. Adapt, don't abandon. |
| Strategy construction reveals that max loss exceeds risk tolerance after commission | Commission drag on smaller strikes or many legs makes the risk/reward unacceptable | First: Widen strike width (reduces commission per dollar of risk). Reduce contract count by 1 and accept lower absolute return. If that fails: Switch to a strategy with fewer legs — iron condor (4 legs) → short strangle (2 legs) if undefined risk acceptable, or vertical spread (2 legs) for defined risk. Last resort: Abort the trade. "Better no trade than a bad trade." If the strategy cannot be constructed within risk tolerance after reasonable adaptations, the setup is not actionable. Flag: "Strategy rejected — risk/reward does not clear commission-adjusted hurdle." | Commission is a real cost that destroys marginal edges. If commission pushes the risk/reward below your threshold, walking away is the winning move. Every strategy must clear the commission-adjusted hurdle or it's not an actionable trade. |
...
1. Always check WHY IV is high. Event-driven IV requires event-specific strategies. Regime-driven IV (market-wide fear) is safer for premium selling.
2. UOA direction + IV rank = strategy. Bullish + high IV = sell puts, not buy calls. Bearish + low IV = buy puts, not sell calls. Direction from UOA, premium buy/sell from IV.
3. Use $5-wide minimum. At $5 wide, the spread has room for partial wins and losses. The commission efficiency alone pays for the wider risk.
4. Close ALL iron condors and credit spreads by 3:30 PM ET on expiration day. The last $0.10 of premium is never worth the pin risk. Close at 21 DTE if anywhere near profit target.
