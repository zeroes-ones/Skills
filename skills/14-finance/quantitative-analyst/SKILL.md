...
## Error Decoder
<!-- QUICK: 30s — common quantitative finance errors and how to fix them -->

| Error | Root Cause | Fix |
|-------|------------|-----|
| **IV calculation diverges at extreme strikes** | Newton-Raphson fails when Vega → 0 for deep ITM/OTM options; the B-S vega is near-zero, causing NR step size to explode | Switch to bisection method on [0.001, 5.0] for strikes where \|delta\| > 0.95 or \|delta\| < 0.05. Bisection is slower but guaranteed to converge. |
| **Put-call parity violation detected** | Usually bad data (stale quote on one leg, dividend not factored, or early exercise premium for American options on dividend-paying stocks). Could be genuine arbitrage on European-style index options | (1) Verify quotes are contemporaneous (same timestamp). (2) Adjust for present value of dividends: C + K·e^(−rT) = P + S₀ − PV(dividends). (3) Check for hard-to-borrow fees on short side. If violation > transaction costs after adjustments, flag as arb opportunity — but this is rare in liquid markets. |
| **Gamma explosion near expiry** | ATM Gamma → ∞ as T → 0 (Gamma ∝ 1/σ√T). This is mathematically correct but produces unrealistically large position Greeks | Cap Gamma at S₀ × 10 for reporting. For position sizing, use 1-day-forward Gamma (recompute with T+1 day) to avoid singularity. Never use raw Gamma for risk limits on <3 DTE positions. |
| **IV rank > 100% or < 0%** | Current IV is outside the 52-week range — common after volatility events (COVID crash, 2008) when the lookback window includes an extreme outlier | Report as "IV at N-year high/low" with absolute IV context. IV Percentile is safer than IV Rank when extreme events are in the window. Consider using a 2-year or 5-year window for mid-caps that have thin options history. |
