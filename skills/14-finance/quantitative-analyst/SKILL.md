...
|---------|-----------|-----|--------|
| Correlation matrix shows 0.92 between AAPL and MSFT — pairs trade loses $80K in 3 months | Correlation computed on price levels, not returns. Both stocks trended up over the period, creating spurious 0.92 correlation. On log returns, actual correlation is 0.15 — no pairs trade edge exists. | Always compute correlations on stationary transforms: log returns for prices, differences for levels. Run Augmented Dickey-Fuller test on every series before correlation, regression, or any statistical inference. p-value > 0.05 = non-stationary = don't use raw series. | Two stocks both going up are not "correlated" — they're both exposed to the same market factor. Correlation on prices is measuring shared trend, not shared movement. Use returns or you're just measuring beta. |
| Backtest Sharpe 3.1 on 500 parameter combinations — strategy loses $4M live in 8 months | p-hacking: at α=0.05, 25 of 500 combinations are "significant" by chance. Researcher selected the best one and declared it a discovery. No multiple-testing correction applied. | Apply Benjamini-Hochberg FDR correction across ALL trials. Report: "After FDR correction at q < 0.05, X of Y strategies remain significant." Use Deflated Sharpe Ratio (DSR) that accounts for number of trials. Require Haircut Sharpe = nominal Sharpe × (1 − selection bias penalty). | A "statistically significant" strategy from 500 trials is expected 25 times by chance alone. The number of trials is part of the result — hiding it is research fraud, even unintentional. |
| Small-cap factor strategy: 22% annualized backtest, 4% live — difference is survivorship bias | Backtest universe used current S&P 600 constituents, not historical. Companies that went bankrupt, were delisted, or dropped from the index were excluded — survivorship bias inflated returns by 2-4% annually over 15 years. | Use point-in-time index constituents from a survivorship-bias-free database (CRSP, Compustat). For every backtest date, the universe includes all securities that existed on that date, including those that later failed. Reconcile universe counts against historical exchange listings. | The companies that don't survive are the ones that would have crushed your strategy's returns. Excluding them is like testing a medical treatment only on patients who lived — the results are meaningless. |
| Max drawdown in backtest: 15%. Max drawdown live: 41% — "the model said it was safe" | Single-path backtest reported point estimate of max drawdown. Monte Carlo with 10,000 paths shows distribution: median 18%, 95th percentile 35%, worst case 52%. The single path was one of the luckiest trajectories. | Report risk metrics as distributions, not point estimates. Run Monte Carlo simulation (10,000+ paths) or block bootstrap. Present: "Max drawdown: median X%, 95th percentile Y%, worst observed Z%." If the 95th percentile drawdown exceeds risk limits, the strategy is not viable regardless of median. | A single backtest path is one possible history. Your live trading will follow a different path — and it won't be the luckiest one. The distribution of outcomes is the result; the point estimate is a marketing number. |
| Signal constructed from 8 features, trained on 2015-2020, tested on 2020-2023 — Sharpe 2.1. Live: Sharpe 0.2 | Feature 3 (options put/call ratio) was computed using look-ahead: the ratio at 3:59 PM was labeled with "today's" date, but options stop trading at 4:00 PM ET while equities close at 4:00 PM — the signal used information not available at trade time. | Timestamp every feature with its PUBLICATION time, not its observation time. For daily strategies: signals computed at time T must only use data published by T-1 market close. Build an automated "time travel detector" that validates no feature value uses data timestamped after the signal generation time. | The hardest bugs to find are the ones where your code is correct but your data labels are wrong. A feature timestamped "January 15" that actually contains January 15 closing data is a time machine — and time machines produce great backtests. |
| 200 strategies tested in walk-forward backtest, 3 "validate" out-of-sample — all 3 fail within 18 months live | Walk-forward overfitting: 200 strategies × 20 windows = 4,000 trials. At p=0.05, 200 false positives expected. The 3 "winners" were noise survivors. Probability of Backtest Overfitting (PBO) was 0.92 — nearly certain the "best" strategy was overfit. | Compute PBO (Bailey & Lopez de Prado) for every walk-forward study. PBO > 0.5 = more likely overfit than not. Require PBO < 0.10 for deployment. Track "how many trials were run to find this strategy" — publish this number alongside the Sharpe ratio. | The number of trials is the denominator of credibility. A Sharpe of 2.0 found after 5,000 trials has less statistical weight than a Sharpe of 1.2 found on the first attempt. The search process IS the result. |
