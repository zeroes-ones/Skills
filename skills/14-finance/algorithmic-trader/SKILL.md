...
/ml-engineer && /algorithmic-trader
...
- **Ignoring market regime changes and deploying a strategy that only works in one environment.** Your mean-reversion strategy posts a Sharpe of 2.8 over 2010-2019 — a decade dominated by low volatility and steady growth. You allocate $500K to it in January 2020. By March, COVID-driven volatility triggers the strategy to add to losing positions as they "mean-revert" further away, and a 35% drawdown in 4 weeks vaporizes $175K. The strategy wasn't broken — it was optimized for a low-volatility regime and had no circuit breaker for VIX > 40. A simple regime filter (trend strength, volatility percentile, correlation matrix stability) would have moved to cash in late February. **Total cost: $100K-$500K in drawdown from regime-blind strategies, often exceeding 2-3 years of the strategy's prior profits in weeks.** Implement a regime detection layer before signal generation: classify the current market as trending/mean-reverting, high-vol/low-vol, crisis/calm using rolling metrics (30-day vs 200-day volatility, cross-asset correlation, trend strength indicators). Each strategy should declare which regimes it's designed for and automatically reduce exposure or go flat when the detected regime doesn't match.
...
|---------|-----------|-----|--------|
| Strategy with Sharpe 3.2 in backtest delivers Sharpe 0.3 live — $2M in 6 months of trading losses | Walk-forward overfitting: 200 parameter combinations tested across 20 windows (4,000 trials). At p=0.05, expect 200 false positives. The "best" parameter set is pure noise. | Apply Deflated Sharpe Ratio (DSR) across ALL trials across ALL windows. Require PBO (Probability of Backtest Overfitting) < 0.10. Out-of-sample period must never have been used for any optimization decision. | The Sharpe ratio of your best backtest is inversely proportional to the number of trials you ran to find it. One backtest = one degree of freedom burned. |
| Live trading P&L shows -3% when backtest says +8% in same market conditions | Transaction cost model uses fixed 5bps slippage from calm-market calibration. In live trading, effective spread widens to 35-85bps during volatility events. 200% monthly turnover at 80bps = 19.2% annual drag. | Model costs as a function of volatility regime: use Almgren-Chriss impact model with spread multipliers (1× normal, 3× elevated VIX, 5× crisis). Circuit breaker: reduce position sizes to 50% when realized costs exceed model by 2×. | The transaction cost model IS the strategy. A brilliant signal minus realistic costs equals a losing strategy. Calibrate costs on your worst market regime, not your average. |
| Mean-reversion strategy posts Sharpe 2.8 over 2010-2019 — loses 35% in 4 weeks during March 2020 | Strategy optimized for low-volatility regime with no circuit breaker for VIX > 40. Added to losing positions as they "mean-reverted" further away during COVID crash. | Implement regime detection layer BEFORE signal generation: classify market as trending/mean-reverting, high-vol/low-vol, crisis/calm using rolling 30d vs 200d volatility. Each strategy declares its compatible regimes and auto-reduces exposure when mismatched. | A strategy without a regime filter is a strategy that only works in the backtest period. The market regime always changes — the only question is when. |
| 11 of 47 orders delayed or rejected during Fed day volatility — $40K loss from positions stuck without stop-losses | Infrastructure not stress-tested for high-volume events. EC2 instance network throughput saturated, broker API rate limiter triggered, stop-loss logic queued behind delayed orders. | Colocate in broker's primary region. Stress-test by replaying historical high-volatility days with real order routing (paper trading). Circuit breaker: flatten positions when order queue exceeds threshold. Budget 3-5× normal slippage for volatility events. | Backtests assume instant, fee-free execution at mid-price. Live trading discovers your infrastructure's failure modes — usually during the worst possible market conditions. |
| Long-short equity strategy: 18% backtest, 4% live — short leg destroyed by borrow costs | 20% of short candidates are hard-to-borrow (fees 15-80% annualized), 8% unshortable entirely. Prime broker recalls 3 positions during short squeeze forcing cover at worst prices. | Before backtesting, obtain historical short availability and borrow cost data (DataLend, S3 Partners). Exclude names with borrow fee > 10% annualized from universe. Reduce position size in stocks with short interest > 20% of float. | The short side of a long-short strategy is not the mirror image of the long side. Borrow costs, buy-in risk, and short-sale constraints are first-order effects, not rounding errors. |
