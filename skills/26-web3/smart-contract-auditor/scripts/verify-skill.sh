...
if grep -q "Portability target:" "$SKILL_MD"; then
  ...
  echo "  [FAIL] Missing portability target"
  FAILURES=$((FAILURES + 1))
fi
...
if grep -q "author: Sandeep Kumar Penchala" "$SKILL_MD"; then
  ...
  echo "  [FAIL] Author field incorrect or missing"
  FAILURES=$((FAILURES + 1))
fi
...
if [ "$GOTCHA_COUNT" -ge 6 ]; then
  echo "  [PASS] Dollar-quantified gotchas: $GOTCHA_COUNT (minimum 6)"
else
  echo "  [FAIL] Dollar-quantified gotchas: $GOTCHA_COUNT (need at least 6)"
  FAILURES=$((FAILURES + 1))
fi
...
REF_LINKS=$(grep -c '(references/' "$SKILL_MD" || echo 0)
if [ "$REF_LINKS" -ge 8 ]; then
  echo "  [PASS] Reference links: $REF_LINKS (minimum 8)"
else
  echo "  [FAIL] Reference links: $REF_LINKS (need at least 8)"
  FAILURES=$((FAILURES + 1))
fi
