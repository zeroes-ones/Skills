...
for s in "Ground Rules" "Anti-Rationalization" "Route the Request" "Core Workflow" \
         "Decision Trees" "Gotchas" "Verification Guardrails" "Error Recovery" \
         "State Log" "References"; do
  check_section "$s"
done
...
COMPLETE_COUNT=$(grep -c "Complete when" "$SKILL_MD" || echo 0)
if [ "$COMPLETE_COUNT" -ge 8 ]; then
  echo "  [PASS] Completion criteria: $COMPLETE_COUNT (minimum 8)"
else
  echo "  [FAIL] Completion criteria: $COMPLETE_COUNT (need at least 8)"
  FAILURES=$((FAILURES + 1))
fi
...
if [ -d "$REF_DIR" ]; then
  for ref in $(grep -oh '[^/]*\.md' "$SKILL_MD" 2>/dev/null | grep -v 'SKILL.md'); do
