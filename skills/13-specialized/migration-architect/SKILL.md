...
workflow:
  artifacts:
    inputs: [migration-scope]
    outputs: [migration-plan]
  completion:
    criteria:
      - Source and target row counts match within 0.01 percent per table
      - Rollback plan tested, with a measured rollback window
      - Cutover runbook lists per-step timing and rollback triggers
    evidence: required
...
|---------|-----------|-----|--------|
| Database migration adds a `NOT NULL` column to a 500M-row table — migration locks the table for 47 minutes, taking the application offline during peak traffic | `ALTER TABLE ... ADD COLUMN ... NOT NULL DEFAULT 'pending'` acquires an `ACCESS EXCLUSIVE` lock for the entire duration. At 500M rows with a mechanical disk, rewriting every row takes 45+ minutes. All reads and writes are blocked for the duration. The migration was tested on a 10K-row staging database and took 200ms | Use `ALTER TABLE ... ADD COLUMN ... DEFAULT 'pending'` (no `NOT NULL`) in one fast transaction, then `ALTER TABLE ... ALTER COLUMN ... SET NOT NULL` in a second transaction after backfilling. Or use `gh-ost`/`pt-online-schema-change` which creates a shadow table and replays binlog events — zero locking | Staging tests lie about production scale. A 200ms ALTER on 10K rows becomes a 47-minute outage on 500M rows. The `NOT NULL` constraint forces a table rewrite. Always separate "add column" from "add constraint" and use online schema change tools for large tables. |
| Cloud migration: lift-and-shift of 200 VMs to AWS completes — first production traffic spike and the entire application slows to a crawl because EBS volumes were provisioned with default `gp2` instead of `gp3` or `io2` | The migration plan specified "equivalent compute" — matching vCPU and RAM. Storage was migrated at the same GB count but the default `gp2` volume type was selected because the migration script used `volume_type: gp2` for compatibility. `gp2` IOPS are capped at 16,000; the application needed 40,000 IOPS burst during peak. EBS burst balances deplete in 30 minutes | Audit IOPS, throughput, and network bandwidth requirements before migration — not just GB and vCPU. Use CloudEndure or AWS MGN with `volume_type: gp3` and `iops: 16000` as baseline. Run a load test that matches production peak traffic BEFORE cutting over. Monitor `VolumeQueueLength` > 1 per volume — means IOPS are throttled | Cloud storage is tiered by performance, not just capacity. A 500GB `gp2` volume has 1,500 baseline IOPS; a 500GB `gp3` volume has 3,000 baseline IOPS (and bursts to 16,000 without extra cost). Default volume types are the slowest — always specify. |
| Framework migration: React 17 → React 18 upgrade passes all tests in CI, deploys to production, and every page with a `useEffect` with no dependency array starts an infinite re-render loop | React 18 changed `useEffect` behavior with Strict Mode — effects run twice in development, revealing missing cleanup functions. But in production, effects with empty dependency arrays `[]` that mutated state caused infinite loops because React 18 batching behavior changed the timing of state updates. Tests passed because Jest doesn't run Strict Mode by default | Run the full test suite with `React.StrictMode` wrapping every component (use `jest.setup.js` to inject it globally). Add an ESLint rule: `react-hooks/exhaustive-deps: error`. Test with React 18's new behavior: wrap renders in `act()` and assert on the number of render cycles (< 3 per update). Run a canary on 5% of production traffic for 24 hours before full rollout | Framework upgrades change runtime behavior in production-specific ways. Strict Mode is the closest approximation of production behavior, but even it doesn't catch everything. Always canary framework upgrades — timing changes in batching and rendering surface under real traffic patterns, not unit tests. |
| Dual-write migration: old system writes to PostgreSQL, new system writes to DynamoDB — 6 months later, 3% of records exist in PostgreSQL but not DynamoDB because writes to DynamoDB were silently throttled | The dual-write implementation had a `try { await dynamoDb.put(params) } catch { log.error(e) }` — it caught the `ProvisionedThroughputExceededException` and logged it, then continued. The DynamoDB table had provisioned capacity at 10 WCU but the write throughput was 50 WCU during peak. 97% of records wrote successfully; 3% were silently dropped | Make dual-write failures BLOCKING: if either system's write fails, the other system's write must be rolled back (or the request must fail with a 500). Use a reconciliation process: every hour, compare record counts and checksum last hour's data between both systems. Alert if reconciliation diverges > 0.01%. Provision DynamoDB with on-demand capacity during migration | Silent dual-write failures are the most dangerous migration failure mode. You think you have two copies; you have 1.97 copies. Reconciliation must run continuously, not as a post-migration validation. If reconciliation diverges, the migration is paused until it converges. |
| Traffic splitting: 1% canary of new service looks healthy — P99 latency is 200ms. Cut over 100% and P99 jumps to 45 seconds because a connection pool sized for 1% traffic exhausts at 100% | The new service's database connection pool was configured with `max_connections: 10`. At 1% traffic, 10 connections was 2× the needed capacity (5 active connections). At 100% traffic, the service needed 200 connections — 10 connections created a queue of 190 waiting requests. Each request waited 45 seconds for a connection | Size connection pools for 100% traffic during migration planning, even though only 1% of traffic is routed. Run a load test at 100% traffic volume against the new service BEFORE starting traffic splitting. Monitor connection pool wait time as a canary gate — if `pool_wait_time > 100ms` at 1%, halt and resize before proceeding to 25% | Canary metrics are proportional to traffic — but resource pools are not. A 10-connection pool at 1% traffic works fine. At 100% traffic, the same pool queues 190 requests. Sizing must be done for the target traffic, not the canary traffic. Resource exhaustion is non-linear with traffic increase. |
...
- **Schema migration without backward-compatible application code.** You add a `NOT NULL` column to the database and deploy the application code in the same release. The migration runs first (ALTER TABLE takes 45 seconds and blocks writes), and during those 45 seconds, old application instances still running cannot insert rows because they don't know about the new column. Three payment transactions succeed in the payment gateway but fail to record in your database, creating an accounting discrepancy that takes 3 days to reconcile. **Total cost: $30,000-$200,000 in failed transactions during migration, data reconciliation labor, and customer-impacting errors from non-backward-compatible schema changes.** Fix: Use the expand-contract pattern — phase 1: add nullable column with default; phase 2: backfill existing rows; phase 3: deploy app reading from new column; phase 4: make column NOT NULL and remove old column. Every schema change must be compatible with the PREVIOUS application version.
- **Performance regression in the target system that wasn't load-tested.** The migration plan covers data integrity, schema compatibility, and DNS cutover — but nobody ran a load test against the new system because "it's the same database version on faster hardware." Post-migration, the new query planner chooses a sequential scan instead of the index scan the old system used because statistics weren't rebuilt. A query that took 50ms now takes 8 seconds, triggering a cascade of timeouts and a full service outage. **Total cost: $50,000-$500,000 in unplanned downtime, emergency rollback, and lost confidence in the migration.** Fix: Run `ANALYZE` on all tables after data migration to rebuild query planner statistics; replay a sample of production queries against the target and compare execution plans; run a full-scale load test at 150% of peak production traffic before declaring migration complete; monitor query performance for at least 72 hours post-migration.
- **Assuming the old system can be decommissioned immediately after cutover.** The migration is declared "complete" at 2 PM on Tuesday, and the old system is scheduled for shutdown at 5 PM. At 4:30 PM, a batch job that processes $450K in daily transactions still points to the old database via a hardcoded IP address nobody documented. The ops team frantically restores the old system from an incomplete backup and spends 16 hours rebuilding the batch pipeline. **Total cost: $50,000-$300,000 in premature decommissioning incidents, emergency restoration labor, and financial reconciliation costs.** Fix: Run the old system in read-only mode for at least one full business cycle (1-4 weeks) after cutover; audit all system connections before decommissioning via network logs and codebase grep for IPs/hostnames; maintain a "dark mode" where the old system is reachable but zero requests hit it for a full cycle before shutdown.
