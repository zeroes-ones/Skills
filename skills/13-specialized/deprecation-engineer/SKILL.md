...
workflow:
  artifacts:
    inputs: [deprecation-candidate]
    outputs: [deprecation-plan]
  completion:
    criteria:
      - Every deprecated surface has a sunset date, a replacement, and a migration guide
      - Zero-invocation confirmed by 30+ days of counter metrics before removal
      - A rollback path documented for every removal
    evidence: required
