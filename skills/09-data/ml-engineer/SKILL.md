...
- **`sklearn.preprocessing.LabelEncoder` is NOT for features.** It's designed for target labels only. Using it on features that have unseen categories in test data throws `ValueError`. Use `OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=-1)` instead.
...
- **`StandardScaler` with outliers produces garbage.** A single outlier at 1,000,000 shifts the mean so far that 99% of scaled values cluster near 0. Use `RobustScaler` (median + IQR) when data has outliers.
...
- **`random_state=42` in `GridSearchCV` doesn't make results reproducible.** `GridSearchCV` uses `random_state` only for shuffling within CV splits, but `n_jobs > 1` makes parallel execution non-deterministic. For true reproducibility, set environment variables: `OMP_NUM_THREADS=1`, `MKL_NUM_THREADS=1`, `NUMEXPR_NUM_THREADS=1`.
...
- **Calling `model.fit()` twice appends estimators, doesn't restart.** In XGBoost/LightGBM, calling `.fit()` a second time continues training with the existing model state. To retrain from scratch, create a new model instance.
...
- **ROC-AUC is misleading for imbalanced data.** On a 99:1 dataset, a model predicting "always majority" gets 0.99 accuracy and ~0.5 AUC. But a model with 0.95 AUC can still have < 10% precision. Always report PR-AUC alongside ROC-AUC for imbalanced problems.
