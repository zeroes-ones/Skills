...
|---------|------------|-----|--------|
| Production database corrupted, last verified backup is 3 months old -- 3 months of customer data lost | Backup ran nightly but restore was never tested; backup file was truncated for the last 3 months due to disk space issues | Implement automated weekly restore-to-staging tests; monitor backup job output for truncation/failure; set up alerting: "backup not verified in 7 days" | An untested backup is a wish, not a backup -- the first restore test should not be during a crisis |
| Application serves stale data: users see orders from 2 hours ago, not current state | Async replication lag spikes to 2+ hours during peak traffic; read replicas serve queries without lag awareness | Implement lag-aware routing: app reads from primary if replica lag > 5s; monitor and alert on lag threshold; add read replica capacity during peak | Replication lag is silent until a customer complains -- if you don't monitor it, you don't have replicas, you have stale copies |
| Traffic spike causes connection pool exhaustion -- app returns 503 errors for 20 minutes | PgBouncer pool sized for average traffic (50 connections) but peak requires 200; no circuit breaker or queueing | Size connection pool for peak × 1.5 headroom; implement connection queuing; add circuit breaker pattern; auto-scale pool on connection count threshold | Pools sized for averages fail during spikes -- capacity planning must use P95/P99 connection counts, not means |
| PostgreSQL refuses new transactions -- "approaching transaction ID wraparound" | Autovacuum default scale_factor (0.2) too high for large tables; vacuum not keeping pace with transaction rate; database forced into single-user mode | Reduce `autovacuum_vacuum_scale_factor` to 0.01 for large tables; monitor `age(datfrozenxid)` and alert at 200M; schedule aggressive vacuum on high-write tables | The default autovacuum settings from 2005 will silently kill your database in 2026 -- tune for your write rate, not the defaults |
| ALTER TABLE added NOT NULL column on a 50M-row table -- production queries blocked for 8 minutes | Adding NOT NULL with DEFAULT rewrites the entire table with an ACCESS EXCLUSIVE lock; no review of locking implications | Use expand-contract: ADD COLUMN nullable → backfill in batches → ALTER COLUMN SET NOT NULL → ALTER COLUMN SET DEFAULT; always review DDL locking impact on production | Locking DDL in production is surgery without anesthesia -- review every schema change against a production-sized dataset before running |
