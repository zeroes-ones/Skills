...
1. **For privacy program management (privacy-engineer)**.
2. **IAM architecture (iam-architect)**.
3. **Or general cloud security (cloud-security)**.
...
This table is specific to `data-security`: each row names a failure this work actually produces, and the response that failure requires.
