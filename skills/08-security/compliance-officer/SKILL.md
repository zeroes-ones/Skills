...
|---|---|
| "SOC 2 Type II means we're secure" | SOC 2 attests to control design AND operating effectiveness over a specific period. It says nothing about the critical CVE from last week, whether your S3 buckets are publicly accessible, or whether controls are actually good — just that they existed and operated as designed. |
| "We passed the audit, so we can focus on other things now" | Compliance is continuous. ISO 27001 requires ongoing risk assessment, monitoring, and annual surveillance audits. The gap between your last audit and today is where breaches happen — Marriott's 4-year-undetected breach cost £99M in GDPR fines. |
