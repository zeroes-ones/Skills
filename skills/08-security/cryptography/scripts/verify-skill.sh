...
echo "--- Frontmatter ---"
if grep -q "^name: cryptography-engineer" "$SKILL_MD"; then
  echo "  [PASS] Frontmatter name field present"
else
...
if grep -q "^portability:" "$SKILL_MD"; then
  echo "  [PASS] Frontmatter portability field present"
else
  echo "  [FAIL] Frontmatter missing portability field"
  FAILURES=$((FAILURES + 1))
fi
