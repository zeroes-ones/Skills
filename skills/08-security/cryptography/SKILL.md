...
|---------|-----------|-----|--------|
| Production service using self-signed certificate — "it's just internal." Vendor integration fails during onboarding, $50K deal delayed 3 weeks | "Internal-only" exemption from PKI policy. Certificate validation disabled in client libraries as workaround. When vendor requires valid TLS, no trusted CA chain exists | Use internal CA (Step CA, HashiCorp Vault PKI, AWS Private CA) with automated issuance via ACME. All services — internal and external — get certificates from the same PKI. Never disable certificate validation; it trains developers that TLS errors are ignorable | "Internal only" doesn't mean "security optional." Internal traffic traverses shared networks, compromised containers, and malicious insiders. Your internal PKI should be as rigorous as your external one — automation makes it free |
| AES-GCM encryption/decryption works perfectly → after 2^32 messages, all ciphertexts become decryptable by an attacker. Customer data exposed, regulator notified | AES-GCM nonce reuse: a fixed IV or counter rollover after 2^32 encryptions under the same key. GCM's security collapses catastrophically on nonce reuse — XORing two ciphertexts with the same nonce reveals the XOR of plaintexts | Use AES-GCM-SIV (RFC 8452) for nonce-misuse resistance — it remains secure even with repeated nonces. If stuck with GCM: implement monotonic counter with key rotation at 2^28 messages (order of magnitude before 2^32 limit). Never use random nonces with GCM — birthday bound at 2^32 is easily reachable in high-throughput systems | AES-GCM's safety margin is razor-thin: one nonce reuse and all security is gone. For any system processing >1M encryptions, GCM-SIV or XChaCha20-Poly1305 is mandatory. The performance difference is negligible; the security difference is infinite |
| "Custom encryption" implemented by dev team — XOR with a fixed 4-byte key. Passes code review because no crypto reviewer assigned | Engineering culture that treats cryptography as "just another algorithm." Developer Googled "simple encryption" and found a StackOverflow answer from 2009. No crypto library approval list, no mandatory architecture review for anything touching encryption | Establish an approved cryptography library list (libsodium, Tink, Web Crypto API). Require architecture review for any code containing `encrypt`, `decrypt`, `cipher`, `AES`, `RSA`. Never let developers choose primitives — provide high-level APIs: `seal(message, key)` and `open(sealed, key)` that encapsulate algorithm selection | Developers should never touch cryptographic primitives directly. Provide `seal()` and `open()` — two functions, zero choices. The moment a developer types `AES.MODE_CBC`, they've already made 3 security-critical decisions they're not qualified to make |
| TLS certificate expires at 3 AM Saturday — 6-hour production outage, on-call engineer can't renew because DNS validation requires access to registrar that only IT has | Certificate auto-renewal configured via ACME client, but the DNS-01 challenge plugin crashed 2 weeks ago and silently stopped renewing. No monitoring on certificate expiry — only detected when clients started getting `CERTIFICATE_EXPIRED` errors | Monitor certificate expiry with 30-day, 14-day, and 7-day alerts. Monitor ACME client health — alert if renewal hasn't happened within expected window (e.g., no renewal in 60 days for 90-day certs). Pre-provision backup certificates with longer validity. Store DNS API credentials in a vault accessible to on-call | Certificate renewal failure is the most common production outage for a reason: it's silent until it's catastrophic. Monitor the renewal process, not just the expiry date. If your ACME client dies, you have 90 days to notice — but you won't, because no one checks |
