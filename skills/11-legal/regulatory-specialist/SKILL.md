...
Complete when: Device classification determined with documented rationale per FDA/EU MDR frameworks. Regulatory pathway selected (510(k)/De Novo/PMA/CE Marking) with timeline and cost estimates. HIPAA applicability determined with Covered Entity or Business Associate status confirmed. Regulatory strategy document produced with predicate analysis and standards gap assessment.
...
- **Missing regulatory horizon scanning.** A medical device company builds to FDA 510(k) requirements today but fails to track the FDA's proposed rulemaking on AI/ML-enabled devices — 18 months later, their entire product line requires reclassification and resubmission because the final rule requires a De Novo pathway for adaptive algorithms. **Total cost: $500K-$5M in retroactive compliance remediation, regulatory resubmission fees, and lost market access during the gap.** Maintain a 24-36 month regulatory horizon scan covering all jurisdictions using structured sources (Federal Register, EUR-Lex, ICH guidelines) with quarterly review cadence.
- **Single jurisdiction assumption in regulatory strategy.** You achieve FDA clearance and assume EU CE marking is a straightforward translation exercise — but the EU MDR requires clinical evaluation reports (CERs) with post-market clinical follow-up (PMCF) plans that FDA never asked for, delaying EU market entry by 9-12 months. **Total cost: $50K-$500K in additional clinical data generation, notified body fees for resubmission, and 9+ months of lost EU revenue.** Design regulatory strategy as a multi-jurisdiction program from day one, mapping requirements across all target markets before locking product design.
- **Compliance managed entirely through manual processes.** A 200-person biotech tracks CAPAs in Excel, manages training records via email, and prepares audit evidence by screenshotting dashboards — an FDA inspection requires 3 weeks of 6-person all-hands preparation and still catches missing signatures. **Total cost: $200K-$1M/year in manual compliance labor and audit prep vs. $50K/year for an eQMS (Veeva, Qualio, Greenlight Guru) with automated workflows.** Deploy a purpose-built electronic Quality Management System that automates document control, training tracking, CAPA workflows, and audit evidence generation.
- **Regulatory filing deadline missed by even one day.** An annual report to the FDA or MHRA is due on March 31 — submitted April 1 at 12:01 AM due to timezone confusion or last-minute fire drill. The clock doesn't restart and late filings accumulate penalties daily. **Total cost: $10K-$100K per day in late filing penalties, plus heightened scrutiny on all future submissions from that regulator.** Maintain a regulatory calendar with 90-day, 60-day, and 30-day pre-alerts, confirmed timezone specifications, and a policy that submissions must be filed 5 business days before the deadline.
...
- **Trusting presentation metadata as authorization.** A capability the client chose to render is treated as a capability the user holds. A user verified for one condition posts data belonging to another, because the endpoint checked authentication and ownership but never the capability itself. **Total cost: $50,000-$500,000 in a privacy breach and regulatory exposure.** Fix: Derive capability on the client for rendering only, and enforce it on the server at every endpoint; add a 403 test per capability. Audit by enumerating every endpoint and asking which capability it requires — as a separate question from what the UI shows.
