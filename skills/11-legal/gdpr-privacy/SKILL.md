...
|---------|------------|-----|--------|
| GDPR fine of 20M EUR for cookie consent violations | Cookie banner had "Accept All" as primary button and "Settings" as secondary -- no "Reject All" option, and analytics cookies fired before consent | Deploy a CMP with: reject-all button equal prominence, granular per-purpose toggles, prior blocking of non-essential cookies, and consent logging. Validate with the DPA's guidance. | A cookie banner without a one-click "Reject All" is not valid consent under GDPR. The CNIL and other DPAs have fined this pattern repeatedly. Equal prominence is a legal requirement, not UX polish. |
| DSAR response took 75 days -- 45 days past the legal deadline | No centralized DSAR engine -- support team manually searched 12 different systems for records, and 3 systems had no search capability | Build a DSAR automation pipeline: identity verification to automated search across all data stores to response assembly and secure delivery. Set up 30-day deadline tracking with automated escalation at day 20. | A manual DSAR process that takes weeks to search multiple systems is a compliance time bomb. By the time you find the data, you have already missed the deadline. Automate before the first request arrives. |
| Laptop containing unencrypted customer database stolen from employee car | No device encryption policy, no data-at-rest encryption on production data exports, and no MDM solution deployed | Implement full-disk encryption on all devices (FileVault or BitLocker), encrypt production database exports, deploy MDM with remote wipe capability, and restrict data exports to authorized, encrypted devices. | Data breaches from lost devices are entirely preventable. If data leaves the secure environment, it must be encrypted. Full-disk encryption is table stakes, not a nice-to-have. |
| EU data transferred to US without SCCs or TIA -- auditor flagged it 18 months later | Engineering team provisioned a US-based cloud service for EU user data without involving the privacy team in procurement | Execute SCCs (2021 version) with the US vendor. Complete a Transfer Impact Assessment (TIA). Add a procurement gate: no data transfer without privacy team sign-off. | Cross-border data transfers do not happen in the legal department -- they happen every time an engineer spins up a database or adds a SaaS tool. Privacy must be embedded in procurement. |
| Data breach notification to DPA submitted on day 6, missing the 72-hour deadline by 3 days | No incident response plan existed for personal data breaches -- the security team spent 4 days trying to determine if the breach was notifiable before notifying the DPO | Create a breach response plan with: 2-hour escalation to the privacy team, 24-hour risk assessment template, and a pre-drafted DPA notification form. Run a tabletop exercise quarterly. | The 72-hour clock starts when you become AWARE of a breach, not when you confirm it. Without a pre-built notification workflow, you will miss the deadline every time. |
