...
4. **Sweep excess cash.** If operating account exceeds 2 months of burn, sweep to interest-bearing reserve account or T-bill ladder. Cash sitting in checking earns 0% — that's a negative real return of ~3-4%.
  Complete when: All bank portals checked, daily cash position documented with variance analysis (<5% flagged), all payments for the day reviewed with approval chain verified, fraud scan completed with zero red flags, and excess cash swept to interest-bearing accounts.
...
4. **Variance analysis.** Each week, compare actual ending cash to forecast. Investigate variance > 5%. Root causes: customer paid late (AR aging problem), vendor billed earlier than expected (AP timing), revenue collected slower (sales or billing issue), unexpected expense (emergency — should be rare).
  Complete when: 13-week rolling cash forecast updated, variance to prior forecast <5% for any given week, and projected cash balance stays above 2 payroll cycles through the entire forecast window.
...
| Gotcha | Cost | Fix |
|--------|------|-----|
| Vendor bank change fraud (BEC) — attacker spoofs executive email requesting wire transfer to "new vendor account"; payment sent to attacker, ACH reversal window missed | $50K-$500K per incident; median loss $85K | Implement mandatory callback verification to a known phone number (not the one in the email) for ALL payment detail changes. Require video confirmation for wires >$50K. Multi-person approval for new payees. Train team: "Any payment change by email is suspicious until verified by phone." Run simulated BEC tests quarterly. |
| Cash concentration risk — all operating accounts, credit facilities, and corporate cards at a single bank; risk department freezes everything during a review, payroll is due in 10 days | Existential — company unable to operate for 30-45 days | Maintain ≥2 active banking relationships with operating accounts at each. Split credit facilities across institutions. Keep 30 days of operating cash at secondary bank. Pre-negotiate standby line of credit at secondary bank. Review concentration risk quarterly: "If Bank A freezes us tomorrow, can we operate for 30 days?" |
| Investment maturity mismatch — operating cash locked in 12-month instruments when needed in 4 months; forced liquidation causes principal haircut and forfeited interest | $100K-$500K per forced liquidation event; relationship damage with lenders | Build cash flow waterfall mapping expected outflows to investment maturities. Use laddered portfolio: 4-week, 8-week, 13-week, 26-week tranches. Never invest operating cash beyond forecasted cash runway horizon. Investment policy must answer: "When do we need this cash?" before "What yield can we get?" |
