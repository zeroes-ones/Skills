...
|---------|-----------|-----|--------|
| Board presentation: P&L shows $500K Q1 net income, CFO presents "great quarter" — Friday: controller discovers they can't make payroll | Modeled P&L without cash flow forecast. $2M AR has 90-day terms, $1.5M AP due in 30 days, $300K Capex next week. Profit on paper, insolvent in reality. The company had a P&L forecast and no cash forecast — these are different documents. | Every financial model must have a 13-week cash flow forecast accounting for AR collection timing, AP payment terms, payroll cycles, and Capex. Cash flow updated weekly, P&L updated monthly. They live in the same workbook with a reconciliation tab showing the bridge from net income to cash position. | Profit is an opinion. Cash is a fact. A company can be profitable for 3 consecutive quarters and file for bankruptcy in month 10 because nobody modeled the gap between when revenue is recognized and when cash arrives. The cash flow forecast is the truth-teller. |
| Board deck: "Revenue growth = 15%" — but headcount tab still uses 12% growth, $3.2M discrepancy between revenue and cost projections | Hardcoded assumptions: "Revenue growth = 15%" typed directly into 47 formulas across 8 worksheet tabs. When assumption changed, 3 cells got updated, 44 didn't. Board presentation contained numbers from two different scenarios — inconsistent numbers caught by a board member during the meeting. | Every assumption lives in exactly ONE cell on a dedicated "Assumptions" tab, colored blue, with source and date annotated. All formulas reference that single cell. Add a "consistency check" sheet verifying key totals match across tabs. Have a second analyst rebuild the model independently and compare outputs before board presentation. | A single "number doesn't match" moment in a board meeting destroys CFO credibility. The board doesn't remember the 47 correct numbers — they remember the one that was wrong and assume the rest are too. |
| "Total revenue = $10M next quarter, up 8% YoY" — but Product A declining 30% (losing $1.5M) masked by Product B growing 40% | Revenue forecast at too high an aggregation level. The net looks healthy. The components reveal Product A's decline — which, if caught 2 quarters earlier, could have been addressed with a pricing change. Instead, Product A loses its last $500K customer without replacement pipeline. | Forecast at the lowest meaningful level: product line × region × channel. Aggregate for reporting, analyze at the component level. Flag any component with > 10% quarter-over-quarter decline for deep-dive variance analysis within 48 hours. The component-level story is the real story. | Aggregate revenue growth is a headline, not an analysis. "Up 8%" can mean "one product doubling while another collapses." If you're only reading the headline, you're missing the fire until it reaches your desk. |
