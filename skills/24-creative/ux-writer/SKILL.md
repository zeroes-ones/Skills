...
|---------|-----------|-----|--------|
| Error message "Invalid input" appears on 14 different form fields — users call support for every one | The developer used a single generic error string for all validation failures. The user has no idea which field is wrong, why it's wrong, or how to fix it. | Write field-specific error messages that identify the field, state the problem, and provide the fix: "Phone number must include area code (e.g., 555-123-4567)." Maintain an error message style guide with the pattern: [what's wrong] + [why] + [how to fix]. | "Invalid input" transfers the debugging burden to the user. Every error message should answer: what field, what's wrong, how do I fix it. |
| Button copy "Submit" tested at 3.2% conversion. "Get my free quote" tested at 11.7%. Nobody can explain why. | The team spent 2 hours debating button copy in a meeting. Nobody suggested testing. "Submit" triggers bureaucratic anxiety; "Get my free quote" communicates value and ownership. | Never debate copy — test it. Run A/B tests on CTA copy, headlines, and error messages. Maintain a "copy wins" log that documents which variants won and why (hypothesis-driven). | Copy decisions made by committee are guesses. Copy decisions made by A/B tests are facts. Test, don't debate. |
| Onboarding flow has 47% drop-off at step 3. The screen just says "Almost done!" but lists 8 more steps. | "Almost done" sets the expectation of 1-2 remaining tasks. When the progress bar reveals 8 more screens, users feel deceived and abandon. The copy created a trust violation. | Align microcopy with actual remaining effort. Use honest progress indicators: "Step 3 of 11" rather than "Almost done." If the process is long, break it into phases with save points. | Copy that lies about progress destroys trust faster than no copy at all. Be honest about what remains. |
| Accessibility audit fails because screen reader announces "unlabeled button" for the primary CTA | The button uses an SVG icon with no `aria-label`. The visual affordance is clear but the screen reader has no text to announce. | Add `aria-label="Submit application"` to every icon-only interactive element. Write button labels that describe the action, not the icon: "Search" not "Magnifying glass." Include screen reader testing in the copy QA checklist. | Copy lives in two channels: visual and auditory. If the screen reader says "unlabeled button," your copy is incomplete regardless of what's on screen. |
| Internationalization: German translation overflows all buttons because German words are 30% longer on average | The English UI was designed with fixed-width buttons. The German string "Einstellungen speichern" is 23 characters vs. 14 for "Save settings" — it doesn't fit. | Design UI components to accommodate text expansion: allow 30-40% width growth for German, 15-20% for French/Spanish. Use `overflow-wrap` and allow buttons to grow. Provide character limits to translators at design time, not after layout is frozen. | Never design UI at the width of English copy. German will break your layouts, and Japanese will break your line-height assumptions. |
