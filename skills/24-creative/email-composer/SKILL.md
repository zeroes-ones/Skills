...
|---------|-----------|-----|--------|
| Marketing email goes out with "Dear {FIRST_NAME}" because the fallback value wasn't tested for empty merge fields | The merge tag `{FIRST_NAME}` has a default fallback of "there" but the QA test only checked populated records. 400 recipients had null first_name in the CRM and received the raw tag. | Always test with null, empty string, and Unicode values for every merge field. Set a visible default: "Dear Customer" instead of relying on the field to always be populated. Add a pre-send validation script that scans for unrendered merge tags. | The merge tag template works exactly 100% of the time — in your test data. Production data is messier. |
| Email looks perfect in Apple Mail but is completely broken in Outlook (Windows) | Outlook uses Microsoft Word's HTML rendering engine (not a web engine), which doesn't support CSS grid, flexbox, background images in divs, or any CSS property added after ~2010 | Use table-based layouts for all email HTML. Inline all CSS. Test in Litmus or Email on Acid across Outlook 2016, 2019, and 365. Add conditional comments for Outlook-specific fixes: `<!--[if mso]>`. | Email rendering is frozen in 2010 for ~30% of recipients. Modern CSS is not available. If it works in Word, it works in Outlook. |
| A/B test shows subject line A won with 28% open rate but actual revenue from line B's cohort was 3x higher | Open rate measures curiosity, not purchase intent. Subject line A was clickbait ("You won't believe this deal") that attracted opens but not the right audience. Subject line B was specific ("Your Q3 savings report is ready") and attracted qualified buyers. | Measure downstream metrics, not just opens. Prize click-to-conversion rate and revenue-per-recipient over open rate. A lower open rate with higher conversion is better than the inverse. | Open rates are a vanity metric. The subject line that drives the most revenue rarely has the highest open rate. |
| CAN-SPAM complaint rate spikes to 0.8% (3x the acceptable threshold) because the unsubscribe link was below the fold on mobile | On iPhone SE, the email rendered at 1800px with the unsubscribe footer at pixel 1400. Users scrolled past 3 screens of content, couldn't find the unsubscribe link, and hit "Report Spam" instead. | Place the unsubscribe link in the header, not just the footer. Test on the smallest supported screen size. Keep emails under 800px total height for mobile. Monitor complaint rate weekly — if it exceeds 0.1%, investigate immediately. | Gmail's spam filter watches complaint rate. One bad email design decision can tank your domain reputation for weeks. |
| Transactional email (password reset) gets caught in the promotional spam folder because it shares an IP pool with marketing blasts | The team used the same SendGrid account for both transactional and marketing emails. Marketing emails have higher complaint rates, which drags down the IP reputation for all sends. | Separate transactional and marketing email streams: different sub-accounts, different IP pools, different sending domains if possible (e.g., `mail.example.com` for transactional, `updates.example.com` for marketing). Transactional emails need near-perfect deliverability. | Never share IP reputation between transactional and marketing email. A password reset email that lands in spam is a support nightmare. |
...
3. **Mobile-first templates — 60%+ of email opens happen on phones.** Design single-column layouts by default. Body text minimum 16px (iOS auto-zooms anything smaller, breaking layouts). Tap targets (CTAs, links, buttons) minimum 44×44px per Apple HIG and Material Design. Test rendering in Litmus or Email on Acid across: iOS Mail (40%+ mobile share), Gmail Mobile (Android), Outlook Mobile, and Apple Mail dark mode. A beautiful 3-column desktop layout that stacks to an unreadable mobile mess loses 60% of your audience before they scroll. Default to single-column with max-width 600px.
4. **CAN-SPAM and GDPR compliance — not legal's problem, it's a deliverability requirement.** Every commercial email must include: (a) a visible, functioning one-click unsubscribe link, (b) the sender's physical postal address in the footer, (c) a non-deceptive subject line and "from" name, and (d) for GDPR, documented proof of consent with timestamp, source, and scope. CAN-SPAM violations carry fines of up to $51,744 per email. GDPR violations: €20M or 4% of global revenue. Beyond fines: ISPs flag non-compliant senders, killing deliverability so even compliant sends never reach inboxes.
5. **A/B testing methodology — isolate one variable per test.** Test one element at a time: subject line (variant A vs B), CTA copy, CTA placement, send time, or personalization approach. Running a test with different subject lines AND different CTAs AND different send times confounds the results — you learn nothing about which change drove the outcome. Minimum sample size: 1,000 recipients per variant for statistical significance at 95% confidence. Run tests for minimum 24 hours to capture full time-zone and open-behavior cycles. Winner becomes the new control for the next test.
6. **Segmentation strategies — relevance beats volume every time.** Segment recipients by behavior (opened last 3 emails? clicked in last 30 days? purchased in last 90 days?), demographics (role, industry, company size), and lifecycle stage (new subscriber, active user, lapsed, churned). A targeted 500-recipient send to an engaged segment outperforms a 5,000-recipient blast to an unsegmented list. Segmentation increases open rates by 14-30% and click-through rates by 50-100%. The cost of segmentation is upfront list hygiene; the cost of not segmenting is burning your sender reputation on disengaged recipients.
7. **Personalization techniques — go beyond "Dear [FIRST_NAME]".** Use behavioral personalization: reference their last purchase, their recent support ticket, the webinar they attended, the feature they used most. Contextual personalization outperforms name-dropping by 6x. A subject line like "Your dashboard shows 3 unassigned leads this week" converts better than "John, check out our new features." Always include a fallback value for every merge tag — "Dear Customer" beats "Dear [FIRST_NAME]" when the data is missing. Test personalization tokens before sending to catch null values, encoding errors, and edge-case character sets.
8. **Send-time optimization — match the recipient's rhythm, not yours.** Default baseline: Tuesday, Wednesday, or Thursday at 10-11 AM in the recipient's local time zone. Avoid Monday mornings (inbox triage backlog), Friday afternoons (weekend mental checkout), and weekends (unless B2C behavioral data shows otherwise). Use send-time optimization tools that learn individual recipient open patterns. A send at 10:17 AM Tuesday (when a specific executive always clears email) outperforms a 10:00 AM batch blast. The difference between optimal and suboptimal send time is 15-25% open rate variance with identical subject lines and content.
9. **Dark mode compatibility — not cosmetic, it's a rendering requirement.** 81.9% of iPhone users and 70%+ of Android users browse in dark mode. Without `@media (prefers-color-scheme: dark)` in your email CSS, white-background emails with transparent PNG logos render invisible text on invisible backgrounds. Key rules: (a) add a dark mode media query that inverts text to light-on-dark, (b) provide an opaque-background logo variant for dark mode (not transparent PNG), (c) test button contrast — brand-colored buttons that pass contrast on white may fail on the dark mode background color the email client auto-generates, (d) avoid CSS `background-image` for hero sections in dark mode — they either don't render or clash with auto-inverted text.
...
|--------------------------|------------|-----|--------|
| Spam complaint rate > 0.1% — Gmail Postmaster flags domain, deliverability plummets | Recipients marking emails as spam instead of unsubscribing. Causes: irrelevant content, too-frequent cadence, purchased lists, or missing easy unsubscribe | Immediately suppress complaining recipients. Audit: list source (purchased? organic?), send frequency, content relevance. Implement confirmed opt-in (double opt-in) for all new subscribers. Move to less-frequent cadence for at-risk segments. Monitor Google Postmaster Tools daily | Complaint rate is the #1 deliverability metric — it overrides open rate, click rate, and every other vanity metric. Above 0.1%, inbox placement drops exponentially. One purchased list can permanently damage a domain's sender reputation |
| Dark mode renders email unreadable — white logo on white background, text invisible | Transparent PNG logo on a white/light background; email client inverts background to #1a1a1a but leaves text and logo colors unchanged | Add `@media (prefers-color-scheme: dark)` CSS block that: (a) sets `background-color` on body/wrapper, (b) provides opaque logo variant via content swap or CSS filter, (c) inverts button contrast. Test in Apple Mail dark mode, Outlook dark mode, and Gmail dark mode (three different rendering engines) | Dark mode is not a "color invert" — each email client implements it differently. Apple Mail auto-inverts light backgrounds but preserves brand colors; Gmail inverts everything; Outlook strips most CSS. Test across all three engines, not just one |
| Reply-All disaster — sensitive message broadcast to entire distribution list | Sender clicks "Reply All" on a message with 50+ recipients, intending to reply only to the sender. Emotional or confidential content reaches unintended audience | Enable Gmail "Undo Send" at 30 seconds (not default 5). For Outlook, configure "Delay Delivery" rule: all emails sit in Outbox 1 minute. Train muscle memory: before every send, scan the To/CC/BCC fields as the last pre-flight check | The Reply-All button has no undo — the 30-second Gmail window catches typos, not judgment errors. A 60-second Outlook rule catches both. One Reply-All mistake at a 500-person company wastes ~$5,000 in productivity interruptions |
| Email lands in Gmail Promotions tab instead of Primary inbox | Gmail's tab-classification algorithm detects: heavy image-to-text ratio, multiple CTAs, promotional language patterns, tracking pixels, list-unsubscribe header presence | Reduce image-to-text ratio below 40:60. Use plain-text-style HTML with minimal styling. Remove tracking pixel for initial engagement emails. Ask engaged recipients to "Move to Primary" (Gmail learns from this signal). Avoid words that trigger promotional classification: "sale," "discount," "offer," "free," "buy now," "limited time" in subject and first 200 body characters | Gmail's tab classifier is a neural network trained on engagement signals — it's not a rules engine you can trick with word substitutions. The only reliable path to Primary is: high engagement rate (opens, replies, "move to Primary" actions), low image-to-text ratio, and plain-text-dominant design |
...
- **Sending emails outside business hours creates a culture of 24/7 availability that burns out teams and damages your reputation as a leader.** A VP who sends emails at 11 PM and on weekends signals "I expect responses at any hour" — even if the VP says "no need to respond now," the power dynamic makes the implicit expectation explicit. Research shows that after-hours emails from leaders increase team burnout scores by 25-35% within 6 months and correlate with 15-20% higher attrition among high performers who value work-life boundaries. A single burned-out senior employee who leaves costs $150K-$250K to replace, and the cultural damage affects the entire team's productivity. **Total cost: $100K-$300K/year in burnout-driven attrition and productivity loss from after-hours email culture.** Use email scheduling (Gmail Schedule Send, Outlook Delay Delivery) to compose when convenient but deliver during business hours. If you must work late, save drafts and send at 8 AM. The only exception: genuine emergencies where a phone call would be faster anyway.
- **Overusing CC as organizational CYA — "I put it in an email, it's not my problem" — creates inbox paralysis and dilutes accountability.** When every decision requires 6 people on CC "for visibility," nobody knows who's actually responsible for action. Recipients assume someone else will respond (bystander effect), the thread grows to 40 messages with zero decisions, and the actual accountable person's action item is buried. A 200-person company where 30% of emails have unnecessary CCs wastes approximately 15-20 minutes per employee per day processing irrelevant email — that's 50-67 hours/day of company-wide productivity loss. **Total cost: $250K-$500K/year in wasted productivity from CC-overload at a 200-person company.** Before CC'ing, ask: "Does this person need to take action, or am I just covering myself?" If the latter, don't CC. Use To: for action required, CC: for information only (rarely), and BCC: for mass distribution. Consider Slack/Teams for status updates that don't need email permanence.
- **Forwarding an email thread without removing internal commentary, prior replies, or confidential context has leaked strategy, compensation data, and personnel discussions to customers, partners, and competitors.** A salesperson forwards a pricing discussion thread to a prospect that includes internal margin notes ("we can go as low as 40% discount — our floor is $85K"). An engineering manager forwards a feature timeline to a customer that includes an earlier message: "this project is severely under-resourced and will probably slip 3 months." These incidents happen weekly at mid-size companies and are almost never caught before the damage is done. **Total cost: $50K-$500K+ per incident — lost deals from exposed pricing floors, lost customer trust from exposed internal assessments, legal exposure from accidentally shared personnel data.** Before forwarding any thread: scroll through the entire history. Delete every message not directly relevant to the recipient. If you wouldn't want the recipient to read it, it shouldn't be forwarded. Better: start a fresh email and summarize, rather than forwarding threads at all.
...
- [ ] **[EM1]** Subject line under 50 characters, front-loaded with value in the first 35 characters, tested with A/B variant on 1,000+ recipient segment
- [ ] **[EM2]** Preheader text explicitly set (not auto-populated with "View in browser" or URL string) — complements the subject line, adds new information, 40-100 characters
- [ ] **[EM3]** Mobile rendering verified in Litmus or Email on Acid across iOS Mail (Apple), Gmail Mobile (Android), and Outlook Mobile — single-column layout, 16px minimum body text, 44px minimum tap targets
- [ ] **[EM4]** Dark mode compatibility: `@media (prefers-color-scheme: dark)` CSS block tested with opaque-background logo variant, button contrast validated, and background-image hero sections verified or replaced
- [ ] **[EM5]** Accessibility audit passed: semantic HTML heading hierarchy (`<h1>` → `<h2>`), `role="presentation"` on all layout `<table>` elements, descriptive alt text on every image, color contrast ≥4.5:1 for all body text
- [ ] **[EM6]** CAN-SPAM/GDPR compliance: physical postal address in footer, one-click unsubscribe link functional and visible, documented consent history (timestamp + source + scope) for every recipient, non-deceptive "from" name and subject line
- [ ] **[EM7]** Personalization tokens tested with fallback values for null/missing data — no "Dear [FIRST_NAME]" or broken merge tags in any test send; seed list includes null records and special-character edge cases
- [ ] **[EM8]** All CTAs have 44px minimum tap targets, are visually distinct from surrounding text, and render correctly with images disabled (bulletproof buttons using padding + border, not image-based buttons)
- [ ] **[EM9]** Attachments under 10MB or replaced with download links; file names are descriptive and recipient-appropriate ("Q3_Report_2026.pdf" not "final_v3.pdf"); sensitive attachments scrubbed of metadata and revision history
- [ ] **[EM10]** Send time optimized: recipient time zone respected, Tuesday-Thursday 10-11 AM local time as default baseline unless behavioral data indicates otherwise, schedule-send used for after-hours composition
- [ ] **[EM11]** A/B test configured (if applicable): single variable isolated (subject line OR CTA OR send time — not multiple), minimum 1,000 recipients per variant, 24-hour minimum test duration, success metric defined before send (open rate, click rate, conversion)
