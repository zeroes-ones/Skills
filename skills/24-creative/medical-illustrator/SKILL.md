...
|---------|------------|-----|
| Clinical reviewer rejects illustration | Anatomical inaccuracy, wrong staging, outdated reference | Verify against latest edition of anatomical reference, re-submit with citation trail |
| Patients misunderstand visual | Too much detail, no visual-first design, text-dependent | Simplify to core concept, test with 5 patients, iterate until >80% comprehension |
| FDA requests illustration revision | Missing disclaimer, unsubstantiated claim, non-standard representation | Add required disclaimers, provide substantiation reference, align with FDA labeling guidance |
| Accessibility audit fails | Color-only differentiation, low contrast, missing alt text | Add patterns/labels to color, increase contrast, write descriptive alt text |
