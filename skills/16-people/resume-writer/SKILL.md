...
1. **For interview preparation** → route to `interview-coach`.
2. **Job search strategy** → route to `job-search-strategist`.
3. **Salary negotiation** → route to `job-search-strategist`.
4. **Or portfolio website design** → route to `frontend-developer`.
...
This table is specific to `resume-writer`: each row names a failure this work actually produces, and the response that failure requires.
