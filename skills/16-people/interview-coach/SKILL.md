...
1. **For resume building** → route to `resume-writer`.
2. **Job search strategy** → route to `job-search-strategist`.
3. **Salary negotiation after offer** → route to `job-search-strategist`.
4. **Or career counseling** → route to `hr-manager`.
...
This table is specific to `interview-coach`: each row names a failure this work actually produces, and the response that failure requires.
