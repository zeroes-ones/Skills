...
2. **Interview preparation** → route to `interview-coach`.
3. **Or personal financial planning** → route to `personal-finance`.
...
This table is specific to `job-search-strategist`: each row names a failure this work actually produces, and the response that failure requires.
