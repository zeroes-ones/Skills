#!/usr/bin/env bash
# verify-skill.sh — Template compliance check for fintech-ui-designer
set -euo pipefail
SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SKILL_FILE="$SKILL_DIR/SKILL.md"
PASS=0; FAIL=0
check() { if grep -q "$2" "$SKILL_FILE"; then PASS=$((PASS+1)); else echo "  [FAIL] $1"; FAIL=$((FAIL+1)); fi; }
echo "=== Verifying fintech-ui-designer ==="
...
echo ""
if [ "$FAIL" -eq 0 ]; then echo "✅ fintech-ui-designer: ALL CHECKS PASSED"; else echo "❌ fintech-ui-designer: $FAIL FAILURES"; fi
exit $FAIL
