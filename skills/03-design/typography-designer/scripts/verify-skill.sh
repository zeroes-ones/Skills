#!/usr/bin/env bash
# Verification harness for typography-designer
#
...
echo "=== Verifying typography-designer ==="
echo ""
...
check "name matches directory" grep -q '^name: typography-designer$' "$S"
...
check "asserts rem over px (R2)" grep -qiE '`rem` for all text roles|author every text size in `rem`|rem. for all text roles' "$S"
check "asserts metric-matched fallback (R1)" grep -q 'metric-matched' "$S"
check "asserts size-adjust mechanism (R1)" grep -q 'size-adjust' "$S"
check "asserts declared base and ratio (R3)" grep -qi 'base and (the )?ratio\|base and a ratio' "$S"
check "asserts licence per context (R4)" grep -qi 'licence.*context\|context.*licence' "$S"
check "asserts coverage proven per shipped file (R5)" grep -qi 'shipped file' "$S"
check "asserts joined scripts must not be tracked (R5)" grep -qi 'joined script' "$S"
check "asserts 1.4.12 spacing overrides (R6)" grep -q '1.4.12' "$S"
check "asserts 200% resize (R2)" grep -q '200%' "$S"
check "forbids zoom disablement (R2)" grep -q 'user-scalable=no' "$S"
