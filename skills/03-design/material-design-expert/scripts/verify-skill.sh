#!/usr/bin/env bash
# verify-skill.sh — Template compliance check for material-design-expert
set -euo pipefail
SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SKILL_FILE="$SKILL_DIR/SKILL.md"
PASS=0; FAIL=0
check() { if grep -q "$2" "$SKILL_FILE"; then PASS=$((PASS+1)); else echo "  [FAIL] $1"; FAIL=$((FAIL+1)); fi; }
echo "=== Verifying material-design-expert ==="
...
echo ""
if [ "$FAIL" -eq 0 ]; then echo "✅ material-design-expert: ALL CHECKS PASSED"; else echo "❌ material-design-expert: $FAIL FAILURES"; fi
exit $FAIL
