...
**What good looks like:** Requirements document with 5 specific constraints (compute, I/O, power, environmental, cost) — all quantified with ranges, not absolutes.
  ...
  Complete when: Requirements document with 5 quantified constraints (compute, I/O, power, environmental, cost) is complete; all constraints have ranges not absolutes; document is reviewed and signed off by hardware lead.
...
**What good looks like:** Selection document with 5 processor candidates, scored on 7 criteria (performance, power, price, ecosystem, availability, security, second-source), with the winner and runner-up documented. A new engineer understands why this chip was chosen.
  ...
  Complete when: Selection document with 5 processor candidates scored on 7 criteria is complete; winner and runner-up are documented with rationale; second-sourcing options with lead time assessment are identified; selection is reviewed and approved by hardware architect.
...
**What good looks like:** Memory map document: base address, size, purpose, and timing requirements for every memory region. No region with "TBD" size.
  ...
  Complete when: Memory map document with base address, size, purpose, and timing for every region is complete; no region has "TBD" size; storage selection (eMMC/NAND/SDCard) with rationale is documented; total RAM usage is measured from prototype, not estimated.
...
**What good looks like:** Power tree diagram showing every voltage rail, the regulator feeding it, maximum current, sequencing order, and sleep mode state. Measured power consumption at each state (active/idle/sleep/deep sleep) within 10% of estimate.
  ...
  Complete when: Power tree diagram showing every voltage rail, regulator, max current, sequencing order, and sleep state is complete; measured power consumption at each state (active/idle/sleep/deep sleep) is within 10% of estimate; battery management strategy (charge IC, fuel gauge, protection) is documented.
...
**What good looks like:** PCB stackup document with layer stack, target impedance, critical net list, decoupling strategy, and placement guidance. Layout engineer can start routing with zero questions about constraints.
  ...
  Complete when: PCB stackup document with layer stack, target impedance, critical net list, decoupling strategy, and placement guidance is complete; layout engineer can start routing with zero questions about constraints; paper design review with layout engineer is completed before routing begins.
...
| Gotcha | Cost | Fix |
|--------|------|-----|
| Component selected without verifying lifetime availability — EOL'd during prototype phase | $150K–$500K per single-source EOL event (redesign, board respin, tooling, lost production) | Check lifecycle status & projected availability at design-in; require second-source or pin-compatible alternative; subscribe to manufacturer PCN alerts; quarterly BOM health review flags parts within 12 months of EOL |
| Power budget calculated at nominal values but peak draw during radio TX or inrush causes brownout reset | $75K–$300K in board respin, power supply redesign & delayed production ramp | Budget for peak current not average; measure inrush & TX burst with current probe; add bulk capacitance for transient loads; 20% margin above worst-case measured peak on all rails |
| EMI/EMC failures at certification lab requiring board respin | $50K–$200K+ in lab retest fees, board respin, tooling & 8–16 week schedule slip | Pre-compliance testing before formal lab submission; minimize hot-loop area in switching regulators; add ferrite beads & common-mode chokes on I/O; maintain 6dB margin to regulatory limits in pre-scan |
