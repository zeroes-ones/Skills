...
|---------|-----------|-----|--------|
| FHIR integration returns valid JSON that the EHR rejects with "unknown resource type" | The EHR vendor's FHIR implementation is on R4 v4.0.0 but your client sends R4 v4.0.1 — the profile URLs changed between patch versions and the EHR's strict parser rejects unknown canonical URLs | Pin FHIR versions explicitly in your client. Query the EHR's CapabilityStatement endpoint and validate supported profiles before sending. Maintain a version compatibility matrix per EHR vendor. | FHIR R4 is not one standard — each minor version and each vendor's "R4-compatible" implementation is a distinct dialect. Test against the actual EHR instance, not the FHIR spec. |
| Clinical decision support alert fires on 97% of patients, so clinicians ignore it entirely | The alert rule uses a broad SNOMED CT code that matches nearly every adult patient (e.g., "finding related to cardiovascular system" instead of the specific "acute myocardial infarction") | Review alert logic with a clinical informaticist who understands the code hierarchy. Use the most specific code in the SNOMED ontology. Add exclusion criteria: age range, lab values, medication history. Test alert rate against a de-identified production dataset — target <5% trigger rate. | An alert that fires on everyone fires on no one. Alert specificity matters more than sensitivity in CDS design. |
| Data warehouse report shows different patient counts than the EHR for the same query | The EHR counts "encounters" (one patient can have 3 encounters) while the warehouse counts "distinct patients." Nobody documented the grain of each data model. | Define and document the grain of every clinical data mart: patient-level, encounter-level, order-level, result-level. Add a data dictionary column for "counting unit." Validate cross-system counts weekly with reconciliation queries. | "Patient count" means different things in different systems. Always clarify: unique patients or total encounters? |
| HL7 v2 message pipeline silently drops lab results with OBX segments using non-standard observation identifiers | The interface engine's validation schema only allows LOINC codes from a 2018 subset. New COVID-19 lab codes (added 2020) fail validation and the messages route to the dead-letter queue with no alerting. | Audit dead-letter queues weekly. Add monitoring that alerts when message volume drops unexpectedly for a feed — if the lab feed goes from 5000 to 200 messages/hour, something is being dropped. Keep LOINC value sets current with quarterly updates. | Interface engines fail silently. The absence of errors does not mean the absence of dropped messages. Monitor volume, not just errors. |
| NLP model extracts medication names from clinical notes but confuses "discontinue metformin" with "prescribe metformin" | The NER model was trained on prescription orders, not clinical narratives. It doesn't understand negation ("no evidence of"), discontinuation ("stop taking"), or historical context ("previously on"). | Add a negation detection layer (NegEx or ConText algorithm) before entity extraction. Train on clinical narratives annotated for assertion status (present/absent/historical/hypothetical). Validate against a gold-standard corpus of annotated discharge summaries. | NLP in clinical text without negation detection is dangerous. "Patient denies chest pain" and "patient has chest pain" are both about chest pain but mean opposite things. |
...
2. **After each major decision:** Append to the ledger:
   ...
   {
     "timestamp": "ISO-8601",
     "skill": "clinical-informatics-specialist",
...
- **LOINC codes for lab tests** — `2951-2` is "Sodium [Moles/volume] in Serum or Plasma." `2947-0` is "Sodium [Moles/volume] in Blood." Different codes, same analyte, different specimen. A dashboard that groups by LOINC code without specimen type conflates serum and whole blood sodium (normal ranges differ by 2-3 mmol/L). **Total cost: $100,000-$500,000 per year** in clinical decision errors — conflation of specimen types leads to incorrect lab interpretations, triggering unnecessary follow-up tests at $200-$2,000 each.
