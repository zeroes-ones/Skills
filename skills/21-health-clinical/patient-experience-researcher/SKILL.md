...
Complete when:
- Journey map built with timeline, swimlanes (clinical/administrative/emotional/social), and pain points annotated with severity (1-4)
...
Complete when:
- PROM instrument selected with validation evidence matching target population on condition, age, language, and literacy
...
| Gotcha | Cost | Fix |
|--------|------|-----|
| Research without underserved populations — biased products that fail in the real world. Study recruits from academic medical center, missing Medicaid, non-English speaking, and low-literacy populations. Product fails with 40% of target users. | $50K-$200K in biased product design rework and lost trust with underserved communities | Recruit ≥ 30% from underserved populations; conduct interviews in preferred language with community health workers; validate design decisions per segment before development |
| Journey mapping without emotional waypoints — the "flatline" patient journey misses trust-building moments. No emotional data captured, leading to 20-30% lower HCAHPS satisfaction scores. | $500K-$1.5M annually in reduced Medicare reimbursement for a mid-size hospital | Augment every journey map with emotional highs/lows using 1-10 sentiment scale; capture verbatim patient quotes; prioritize improvements at lowest emotional points first |
| Conflating patient satisfaction with patient safety — "happy but harmed" discharge process with 24% readmission rate vs 14% standard. Satisfying elements skip critical safety steps. | $100K-$500K per year in preventable readmissions and CMS penalties (up to 3% of Medicare payments) | Triangulate satisfaction scores, readmission rates, and safety incident reports; use Patient Experience + Clinical Safety Matrix; never improve satisfaction at expense of safety |
| Assuming all patients want the same level of engagement — ignoring that 25-40% of chronic patients are at low activation (PAM Level 1-2). App engagement drops to 22% for overwhelmed patients. | $50K-$200K in disengaged patients, worsened outcomes, and product churn | Segment by activation level using PAM during onboarding; design tiered experiences from single daily action to comprehensive tools; adapt as activation changes |
| Research insight reports filed away instead of driving change — 6 months of research produces 47-slide report that's never referenced in product roadmap. Redundant studies waste budget. | $100K-$300K in redundant research and delayed interventions that compound over time | Map every finding to a specific owner and dated action item within 2 weeks; embed researchers in product teams; require quarterly progress reviews on recommendations |
| Single-timepoint patient interviews — interviewing during active treatment yields different narrative than 2 weeks post-treatment. Timing skews the entire experience understanding. | $50K-$150K in misleading research conclusions and product decisions based on incomplete data | Interview at multiple timepoints (during treatment, 2 weeks post, 3 months post); compare narratives across timepoints; note timing context in all findings |
| HCAHPS scores as sole quality measure — 95% satisfaction can coexist with 30% readmission rate. High satisfaction does not equal good clinical outcomes. | $200K-$1M in value-based reimbursement penalties when satisfaction improvements don't track with outcomes | Always pair satisfaction scores with clinical outcomes (readmissions, adverse events, mortality); use composite experience+outcome metrics for program evaluation |
| "Patient voice" tokenism — inviting one patient to share their story with no resulting design changes. Single patient story is n=1 qualitative data, not design direction. | $25K-$100K in wasted patient engagement effort and missed design improvements from underutilized patient input | Involve 8-12 patients with structured thematic analysis; require design changes traceable to specific patient findings; close the loop with participants on how their input was used |
