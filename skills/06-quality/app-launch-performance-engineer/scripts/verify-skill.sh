#!/usr/bin/env bash
# Verification harness for app-launch-performance-engineer
#
...
echo "=== Verifying app-launch-performance-engineer ==="
echo ""
...
check "name matches directory" grep -q '^name: app-launch-performance-engineer$' "$S"
...
check "names the three launch modes" grep -q 'cold' "$S" && grep -q 'warm' "$S" && grep -q 'hot' "$S"
check "defines TTID" grep -qi 'ttid\|time to initial display' "$S"
check "defines TTFD" grep -qi 'ttfd\|time to full display' "$S"
check "separates TTID from TTFD (R2)" grep -qi 'separate' "$S"
check "asserts mode+device+cache in every figure (R1)" grep -qi 'device class' "$S"
check "asserts per-phase attribution (R3)" grep -qi 'attribut' "$S"
check "asserts numeric budget (R4)" grep -qi 'budget' "$S"
check "asserts a numeric budget per mode" grep -qi 'numeric' "$S"
check "asserts method held constant (R5)" grep -qi 'method' "$S"
check "asserts mode must match the complaint (R6)" grep -qi 'not failing\|wrong mode\|other than the one' "$S"
check "rejects splash-as-fix" grep -qi 'splash' "$S"
check "asserts median and p90" grep -qi 'median' "$S" && grep -qi 'p90' "$S"
check "asserts cold-state process control" grep -qi 'kill\|force-stop' "$S"
check "names static initializers" grep -qi 'static initializer\|static init' "$S"
check "names third-party self-init" grep -qi 'content provider\|self-initial' "$S"
check "names compilation profile" grep -qi 'baseline profile\|compilation profile\|profile-guided' "$S"
check "names the module-graph cost" grep -qi 'module graph\|import graph\|importtime' "$S"
check "names hydration / TTI" grep -qi 'hydration\|time to interactive' "$S"
check "asserts a CI gate" grep -qi 'gate' "$S"
check "asserts noise floor / run count" grep -qi 'noise\|run count\|runs' "$S"
check "cites the published excessive thresholds" grep -qi 'excessive' "$S"
check "asserts attribution before fix" grep -qi 'attribut.*before\|before any fix\|before the fix' "$S"
