...
|---------|-----------|-----|--------|
| Single function with 12 boolean parameters: `process(true, false, true, false, true, false, false, true, false, true, true, false)` — nobody knows what it does | Feature flag accumulation. Each boolean added for a different use case over 2 years. Original author left. The 2^12 = 4096 possible flag combinations — only 4 are valid, 4092 are undefined behavior | Replace booleans with an options object: `process({ validation: true, notify: false, cache: true, ... })`. Better: split into separate functions for each use case. If a function has >3 boolean params, it's doing >1 thing | Boolean parameters are the cheapest way to add a feature and the most expensive way to maintain one. Each boolean doubles the test surface. At 12 booleans, you've built a state machine that nobody can debug. |
| `if (user.role === 'admin' || user.role === 'superadmin' || user.role === 'moderator' || user.department === 'engineering' || user.tenure > 5 || ...)` — 6 conditions spanning 3 concepts | Permission logic accreted over time. Each new requirement added another `||` to the same if-statement. No one refactored because "it works" and touching it might break the authorization system | Extract to a named function: `canAccessDashboard(user)`. Group conditions by concept: role checks, department checks, tenure checks — with inline comments. Add unit tests for every branch. The function name documents intent | An `if` statement with >3 conditions is a function begging to be named. The name tells future readers what question you're asking — the implementation tells them how you answer it. |
| Class `OrderProcessor` has 2,400 lines — 47 methods, 14 private fields. Adding a payment method takes 3 days of tracing | God class accumulated responsibility over years. Every feature "naturally" belonged in OrderProcessor because everything "processes orders." No boundary to push back against new methods | Split by responsibility: `OrderValidator`, `OrderPaymentProcessor`, `OrderNotifier`, `OrderFulfillmentHandler`. Each class has one reason to change. Use composition: OrderProcessor delegates to each specialized class | Classes grow one method at a time. Each addition feels innocent — "it's just 20 lines." At 2,400 lines, those 120 innocent additions created a monster. The time to split a class is when someone hesitates before adding a method. |
| Three different date formatting functions: `formatDate()`, `prettyDate()`, `displayDate()` — each handles timezone differently | Three developers independently solved "I need to show a date" without checking if a utility existed. No shared utility directory, no discoverability | Search for existing utilities before writing new ones: `grep -r "formatDate\|prettyDate\|displayDate" src/`. Consolidate into one `formatDate(date, options)` with Locale and timezone params. Deprecate old functions with `@deprecated` JSDoc pointing to the replacement | Three implementations of the same function is not DRY violation — it's a discoverability failure. If developers can't find the existing utility, they'll write their own. A well-organized utils directory IS documentation. |
| `try { ... } catch (e) { }` — empty catch block swallows errors for 18 months until production data corruption discovered | Original developer added try/catch to suppress a transient network error during development. Forgot to remove it. Code shipped. 18 months of silently swallowed errors in production | Never merge empty catch blocks. If you must catch: log the error at minimum (`console.error('Failed to fetch user preferences', e)`). Use lint rule: `no-empty` or `@typescript-eslint/no-empty-function` for catch blocks. Better: handle the error or propagate it | An empty catch block is a black hole for bugs. The error still happens — you just stop hearing about it. Every catch block must either handle the error, log it, or re-throw it. Silence is not a fix. |
