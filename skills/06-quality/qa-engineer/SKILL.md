...
## Footguns
<!-- DEEP: 10+min — war stories from production QA engineering -->

| Footgun | What Happened | Root Cause | How to Prevent |
|---------|---------------|------------|----------------|
| Inverted test pyramid: 4,700 E2E tests, 200 unit tests — CI ran for 3 hours 15 minutes, and a flaky E2E test was the "boy who cried wolf" that masked 6 real failures | A startup built their test strategy around E2E tests because "they're the only ones that prove it actually works." Over 18 months, the suite grew to 4,700 Cypress tests. CI took 3+ hours. Developers started ignoring CI failures — "it's probably just flaky." When a real payment bug surfaced (race condition in order confirmation), 6 consecutive CI runs showed the failure but the on-call engineer dismissed it as "that flaky test again." The bug reached production and caused 89 duplicate charges before a customer called support. | E2E-first testing was chosen because it was the only testing discipline the team was comfortable with. Unit and integration tests were seen as "optional." Nobody tracked the flaky test rate or CI runtime — they just accepted the degradation as normal. | **Enforce the test pyramid by CI budget, not convention.** Set hard time limits per layer: unit tests < 30s, integration tests < 5 min, E2E tests < 15 min. If any layer exceeds its budget, fail the build and require a test refactor before feature work continues. Track flaky test rate weekly — if >2%, halt feature work until flakiness is fixed. The test pyramid isn't dogma; it's math. E2E tests are O(n²) in maintenance cost per feature. |
| Performance tests added to CI with thresholds copied from a blog post — "P95 < 500ms" — the actual service needed P95 < 80ms for a real-time bidding system | A QA engineer read a blog post about k6 performance testing and added a CI gate: "API P95 response time < 500ms." The service was a real-time ad bidding system where advertisers had a 100ms SLA to respond. A gradual performance regression over 4 weeks took the P95 from 65ms to 420ms. CI stayed green (under 500ms!). But advertisers were timing out — the SLA was 100ms, not 500ms. The company lost $47K in ad revenue over 3 days before the ad ops team correlated the timeout logs with the deployment timeline. | The performance threshold was chosen from a generic benchmark, not from the service's SLA. Nobody asked "what's the actual business requirement?" The threshold was 5× too lenient. | **Derive performance thresholds from business SLAs, not benchmarks.** If the SLO is "99.9% of ad bids respond within 100ms," the CI gate should be "P99 < 90ms" (leaving 10ms for network). Document every threshold with its business justification. Set multiple thresholds: "P95 < 80ms (warning)" and "P99 < 100ms (blocking)." Review thresholds quarterly — business requirements change. |
| Coverage at 92% — all lines of code were executed — but 80% of tests mocked the database layer, so zero queries were actually tested against a real database schema | The team celebrated reaching 92% line coverage. Every test used MockRepository — a layer that returned hardcoded objects without touching the database. The coverage tool saw every line execute. But the actual SQL queries — joins, constraints, indexes — were never tested. A migration changed `user_id` from INTEGER to BIGINT and the mock's hardcoded `user_id: 42` didn't catch that the API was now returning NULL for every user lookup. The PR was merged, coverage was 92%, and user profiles were broken for 4 hours. | Coverage was measured on unit tests only. Integration tests (with a real database) existed but were excluded from the coverage report because "they're too slow." The team gamed the metric: they optimized for coverage percentage, not coverage quality. | **Separate coverage reports by test layer.** Unit test coverage measures logic coverage. Integration test coverage measures system coverage. Set minimums for BOTH: 80% unit coverage AND 60% integration coverage. A line covered only in unit tests (mocked DB) is different from a line covered in integration tests (real DB). Track "effective coverage" = the overlap — code exercised in BOTH unit and integration tests. |
| QA signed off on a release after 200+ test cases passed — 3 P0 bugs found in production within 4 hours because none of the test cases used production-like traffic patterns | A major refactor passed all 200+ QA test cases. QA signed off. Four hours after deployment: (1) the login endpoint buckled under 50 concurrent users because connection pool was sized for 10, not 50, (2) a race condition in the shopping cart caused inventory oversells when 3 users bought the last item simultaneously, (3) the search index rebuild timeout caused cascading 504s. All three bugs were invisible in QA because tests ran sequentially with 1 virtual user on a clean database. | The test environment was a miniature replica — 1/100th the production scale. No load testing, no concurrency testing, no dirty-database testing. The test cases validated functional correctness in isolation, not system behavior under load. | **Add pre-release production-simulation gates.** Run a 15-minute soak test at 50% of peak production traffic. Test with a "dirty" database: start the test suite from a production-like database with 6 months of accumulated data, not a clean seed. Run at least 3 concurrent E2E scenarios to surface race conditions. If you can't simulate production traffic, you're not testing — you're hoping. |
| Flaky test tolerated for 18 months — it became "that test that always fails" and engineers trained themselves to retry CI 3 times before looking at failures | A Playwright test for the file upload flow failed roughly 15% of the time due to a race condition with the file picker dialog. The team added `cy.wait(2000)` and it seemed to help. Over 18 months, the failure rate stabilized at ~12%. Engineers developed muscle memory: CI fails → retry → retry → retry → "oh, it's just the upload test" → merge anyway. Three real failures were merged during this period because everyone assumed the red CI was "just the flaky test." One was a broken payment flow that affected 2,300 transactions. | The team normalized flaky tests as "acceptable technical debt." The `cy.wait(2000)` was a band-aid that made the failure intermittent instead of consistent — intermittent failures are HARDER to fix than consistent ones. Nobody owned the flaky test backlog. | **Quarantine flaky tests immediately — they are more damaging than no tests at all.** Configure CI to automatically move any test that fails >3 times in 100 runs to a separate "quarantine" suite. Quarantined tests run but don't block merge. File a bug for every quarantined test with a P2 severity. Track flaky test rate as a team KPI: >2% = no new features until flakiness drops below 1%. A flaky test is an unreliable sensor — it teaches the team to ignore the alarm. |
