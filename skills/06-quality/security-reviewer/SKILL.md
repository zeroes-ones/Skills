...
|---------|-----------|-----|--------|
| Security review finds XSS in search results — fix applied, retest passes. Same XSS found in 4 other places 3 months later | Review fixed the instance, not the pattern. Developer added `escapeHtml()` to the search results template. The other 4 templates that render user input without escaping were never checked | Pattern-based fix: add output encoding to the template engine globally (e.g., enable auto-escaping in Handlebars/Jinja2). Security review finding template: "Root cause: [pattern] → Fix: [address pattern everywhere], grep: [regex to find other instances]" | One XSS is a bug. Five XSSes is a missing framework feature. If user input can reach the DOM unsanitized in one template, it can in every template. Fix the framework, not the template. |
| Penetration test finds `/.env` publicly accessible — returns AWS keys, database password, Stripe secret | `.env` file committed to repo in 2020 during onboarding ("just for local dev"). `.gitignore` added later but file was already tracked. Server serves static files from repo root | Remove `.env` from git history: `git filter-branch` or `BFG Repo-Cleaner`. Configure web server to block dotfiles: `location ~ /\. { deny all; }`. Use vault/Secrets Manager — never store secrets in files the web server can serve | Committed secrets are forever. Even if you add `.gitignore` later, the file is still in git history. Rotate every secret that was ever in a committed `.env` file. Then configure the web server to reject dotfile requests at the edge. |
| SQL injection via `ORDER BY` clause: `?sort=price DESC; DROP TABLE users;--` — parameterized query doesn't help | Parameterized queries prevent injection in WHERE clauses but ORDER BY/GROUP BY can't be parameterized in most SQL dialects. The app concatenates user input directly into the sort expression | Whitelist allowed sort columns: `const ALLOWED_SORTS = ['price', 'date', 'name']; const sort = ALLOWED_SORTS.includes(req.query.sort) ? req.query.sort : 'id'`. Never concatenate user input into ORDER BY, even if "it's just a column name" | Parameterized queries solve 95% of SQL injection. The other 5% — dynamic ORDER BY, GROUP BY, table names — need whitelist validation. "The query builder handles it" is not true for these cases. |
| JWT token with `"alg": "none"` accepted by server — attacker forges admin token without knowing the secret | JWT library's `verify()` defaulted to accepting `alg: "none"` (no signature). Attacker creates token `{"alg":"none","typ":"JWT"}.{"role":"admin"}.` with empty signature. Library: "alg is none, no verification needed" | Explicitly set `algorithms: ['HS256']` (or your chosen alg) in JWT verify call. Never rely on library defaults. Test: create a token with `alg: "none"` and verify your server rejects it with 401 | JWT library defaults are not secure defaults. Every JWT library had this bug at some point. The fix is one line: specify the algorithm explicitly. Test the `alg:none` attack in your security suite. |
| Dependency `left-pad` unpublished from npm — 30% of internet breaks. Your app is down and it's not your code's fault | No dependency verification or caching. Build pipeline pulls latest from npm on every CI run. When a dependency disappears, your build breaks — and your running production code can't rebuild | Use a package registry proxy (Verdaccio, Artifactory) that caches dependencies. Pin exact versions with lock files. Vendor critical dependencies. CI gate: `npm install --frozen-lockfile` — fail if lock file doesn't match. Monitor dependency health: deprecation warnings, maintainer changes, unusual publish patterns | The open-source supply chain is a dependency you didn't choose. When `left-pad` went down, it took React, Babel, and thousands of other projects with it. Your build should survive an npm outage. |
