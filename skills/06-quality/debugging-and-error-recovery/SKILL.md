...
- **Fixing the next plausible cause instead of ruling out a layer.** A symptom persists, so each plausible cause is fixed in turn — and each fix is genuine but irrelevant. Nobody stops to ask which single observation would eliminate an entire layer. **Total cost: $5,000-$40,000 per incident in engineer-hours and delayed resolution.** Fix: When a fix does not change the symptom, stop fixing and start ruling out. Find the observation that eliminates a whole layer — a request that never starts cannot fail, which rules out networking, DNS, and TLS in one step. The evidence is usually available from the start; the mistake is reaching for a fix before reaching for an elimination.
