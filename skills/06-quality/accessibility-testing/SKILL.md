...
|---|---|
| "We'll add ARIA labels after the feature is built" | Developers never go back to add ARIA; 90% of accessibility bugs are missing labels that take 30 seconds to add during development but months to triage and fix post-launch |
| "The automated scanner says we pass, so we're compliant" | Automated tools catch only 30% of WCAG 2.2 AA criteria; the other 70% require manual testing — screen reader workflows, keyboard traps, focus management, and meaningful alt text |
...
|---------|-----------|-----|--------|
| axe-core reports 0 violations but screen reader user can't complete checkout — "100% automated score" is a lie | Automated tools only catch ~30% of WCAG issues. Missing: focus order is illogical (tab goes from "Add to Cart" to footer links), dynamic content updates not announced, custom dropdown has no ARIA pattern | Run manual keyboard audit: Tab through every interactive element in logical order. Test with NVDA (Windows) + VoiceOver (macOS). Check that `aria-live` regions announce content changes. Use Accessibility Insights for manual checks | An axe-core score of 0 means "no automatically detectable violations," not "accessible." The other 70% — focus management, semantic announcements, interaction patterns — require human testing with actual assistive technology. |
| `aria-label="close"` works in English, meaningless to Spanish screen reader users — international users can't navigate | ARIA labels are hardcoded strings. Screen readers don't translate them. `aria-label="close"` is English-only; a Spanish user hears "close" as an English word in a Spanish interface | Never hardcode ARIA labels. Use i18n keys: `aria-label={t('a11y.close_button')}`. The label translates with the rest of the UI. Test with screen reader language set to your top 3 locales | ARIA labels are user-facing strings that need i18n just like visible text. A screen reader in Spanish mode reading English labels is a broken experience. |
| Color contrast ratio "passes" at 4.52:1 but low-vision users still can't read the text — complaining about "washed out" UI | WCAG AA requires 4.5:1 for normal text, but this is a minimum. Thin font weight (300), small size (14px), and light gray on white technically pass but are practically illegible for low-vision users | Target 7:1 contrast ratio for body text. Use font-weight ≥400 for content text. Test with Color Blindness Simulator and blur test (squint — can you still read it?). Don't design to the minimum | 4.5:1 is a floor, not a target. The difference between passing WCAG and being readable is the font weight, size, and design intent behind the contrast choice. Test with actual users, not just automated checkers. |
| `role="button"` on a `<div>` — clickable with mouse, unreachable with keyboard. No Enter/Space handler | Developer used `<div onclick>` for visual flexibility but didn't implement keyboard interaction. Screen reader announces "button" but keyboard user can't activate it | Use `<button>` — it gets keyboard support for free. If you must use `<div role="button">`, add: `tabindex="0"`, `onKeyDown` handler for Enter and Space, and `aria-pressed` for toggle buttons. Test: can you reach it with Tab and activate it with Enter? | Every `onClick` on a non-interactive element is a keyboard trap waiting to happen. If it behaves like a button, use a `<button>`. The browser gives you 20+ interactions for free. |
| Dynamic content loads via AJAX — screen reader stays silent, user doesn't know new results appeared | New content injected into DOM without `aria-live` region. Screen reader focus stays on the "Search" button. User waits 10 seconds thinking nothing happened | Wrap dynamic content container in `<div aria-live="polite" aria-atomic="true">`. For critical updates (errors, confirmations), use `aria-live="assertive"`. Move focus to the first new result with `ref.focus()` after content loads | Screen readers don't watch the DOM. If you add content without an aria-live region, it's invisible to assistive technology. Every AJAX update needs a live region announcement. |
| Image carousel auto-rotates every 3 seconds — user with cognitive disability can't focus on content before it slides away | No pause/stop control. WCAG 2.2.2 requires a mechanism to pause, stop, or hide any moving, blinking, or auto-updating content that starts automatically and lasts >5 seconds | Add visible Pause/Play button. Auto-rotation must pause on hover and focus. Respect `prefers-reduced-motion: reduce` media query — disable auto-rotation entirely. Announce slide changes via `aria-live` | Auto-rotating carousels fail WCAG on three separate criteria: 2.2.2 (pause/stop), 2.3.3 (animation from interactions), and 3.2.2 (no context change on input). The carousel needs user control, not just visual polish. |
