...
## Footguns
<!-- DEEP: 10+min — war stories from production accessibility testing -->

| Footgun | What Happened | Root Cause | How to Prevent |
|---------|---------------|------------|----------------|
| axe-core passed with zero violations in CI for 8 months — then a blind user reported they couldn't complete checkout because the payment form had no accessible labels | The team relied on axe-core as their sole accessibility gate. axe-core catches ~57% of WCAG issues automatically. The payment form used `<div>` elements with CSS-styled placeholders instead of `<label>` elements — axe-core flagged the missing labels under `label-title-only` but the rule was set to `warn` instead of `error` in the configuration. No one reviewed the warnings dashboard for 8 months. | axe-core was configured with default severity thresholds — `warn`-level rules were not blocking. The team believed "CI is green = accessible." No manual screen reader testing was scheduled because "automation covers it." | **Treat axe warnings as errors in CI.** Set `axe.run({ runOnly: { type: 'tag', values: ['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'] } })` and fail on ANY violation — not just critical/serious. Run a monthly manual screen reader walkthrough of your top 5 user flows using NVDA (Windows) or VoiceOver (macOS). Automation catches ~57% of WCAG issues; the other 43% require human judgment. |
| Lighthouse accessibility score stayed at 98 for 6 months while 47 pa11y violations accumulated in the baseline — the baseline became a "known issues" graveyard | The team configured pa11y-ci to compare against a stored baseline, allowing zero NEW violations. Over 6 months, 47 violations accumulated in the baseline from ignored PR comments, "we'll fix later" promises, and one bulk suppression of 12 violations from a third-party chat widget. The accessibility dashboard showed green because no NEW violations were detected — but the absolute violation count was climbing. | The baseline was treated as a permanent exception list rather than a temporary backlog. There was no mechanism to force baseline reduction over time. The "zero new violations" gate was satisfied while the product became progressively less accessible. | **Track baseline size as a KPI with a reduction SLA.** Add a CI check: if baseline violations > 20, fail the build. Require that every sprint closes at least 10% of baseline violations. Add a dashboard showing: (1) total baseline violations trend, (2) oldest open violation age, (3) violations per page. Set a policy: baseline violations older than 90 days require director approval. The only acceptable trend line for baseline size is down. |
| ESLint jsx-a11y plugin was installed and passing — but a developer shipped a modal with no focus trap because they used `/* eslint-disable jsx-a11y/no-noninteractive-element-interactions */` on 14 components | A developer was under deadline pressure to ship a feature with a complex modal dialog. The ESLint plugin flagged 14 `no-noninteractive-element-interactions` violations (divs with onClick handlers). Instead of fixing them with proper `<button>` elements and `role="dialog"`, they added inline disable comments to every violation. The PR was approved because the reviewer only checked the CI green checkmark — not the ESLint output details. The modal shipped with no focus trap, no Escape-to-close, and no focus restoration. | ESLint disable comments were not tracked or reviewed. The CI pipeline passed because the plugin was installed (checking the box), but it didn't enforce that disable comments require justification. The code review focused on logic, not accessibility. | **Ban blanket ESLint disable comments.** Configure `eslint-plugin-jsx-a11y` with `"no-noninteractive-element-interactions": "error"` and add `eslint-comments/no-use: "error"` to prevent eslint-disable without a specific rule. Use `eslint-disable-next-line` with a required comment explaining the justification. Add a CI check that counts ESLint disable comments per file and fails if any file exceeds 3 disables. Review disable comments in PR the same way you review `TODO` comments — they are debt. |
| Mobile accessibility tests passed on emulator but failed on real devices because TalkBack behaved differently on Samsung vs. Pixel — 8 Android users reported the app was unusable | The Android CI pipeline ran Espresso AccessibilityChecks on an x86 emulator running stock Android. The checks passed. But on Samsung devices (29% of the user base), Samsung's One UI overlay modified TalkBack's focus traversal behavior — elements that were focusable on stock Android were skipped on Samsung. On Pixel devices with Android 14, the new "Read this page" feature triggered an infinite loop on a carousel component. The bug went undetected for 3 weeks because the team only tested on emulator. | Emulator testing assumed identical behavior across OEMs, but Samsung, Xiaomi, and OnePlus all modify accessibility behavior at the OS level. The CI infrastructure only had x86 emulators available — no ARM emulators, no physical device lab. | **Test on at least one physical Samsung device and one Pixel device.** Samsung holds 29% of Android market share and their One UI modifies TalkBack behavior. Set up a physical device lab (or Firebase Test Lab with real devices). Run accessibility checks on both stock Android AND Samsung One UI devices. Add a dedicated "TalkBack focus traversal" test that verifies every interactive element on each screen receives focus in a logical order — this catches OEM-specific behavior differences. |
| Lighthouse CI accessibility score stayed at 97 for 12 months — then a European customer requested EN 301 549 certification and the team discovered 23 of the 47 required criteria were never tested by any automated tool | A SaaS company expanded into the EU public sector and needed EN 301 549 accessibility certification (the European equivalent of Section 508). The team believed their Lighthouse score of 97 and clean axe-core reports meant they were compliant. The EN 301 549 audit tested 47 criteria — 12 of which (including "purpose of controls determinable programmatically" and "focus cursor follows reading order") had no automated test coverage in their pipeline. The audit found 23 failures. Certification was delayed by 7 months while the team retrofitted manual testing for the uncovered criteria. | The team assumed WCAG 2.1 AA compliance (what Lighthouse/axe-core test) covered all regulatory requirements. EN 301 549 adds criteria beyond WCAG: biometrics accessibility, interoperability with assistive technology, documentation accessibility, and real-time text communication. Automated tools only test ~57% of WCAG criteria — and an even smaller fraction of EN 301 549. | **Map your testing coverage to the SPECIFIC regulation you need to comply with.** Create a compliance matrix: list every criterion in your target standard (WCAG 2.1 AA, EN 301 549, Section 508), and mark which are covered by automated tools (axe, Lighthouse, pa11y), which require manual testing, and which are not applicable. Review this matrix quarterly as regulations evolve. If you sell into a regulated market, hire a certification consultant BEFORE you need the certification — they'll identify gaps 12+ months before the audit.
