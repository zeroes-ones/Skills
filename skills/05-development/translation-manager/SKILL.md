...
## Footguns
<!-- DEEP: 10+min — war stories from production translation management -->

| Footgun | What Happened | Root Cause | How to Prevent |
|---------|---------------|------------|----------------|
| DeepL API cost went from $400/month to $4,100/month overnight — an auto-retranslation loop re-translated 180,000 strings that hadn't changed | The CI pipeline was configured to push ALL source strings to TMS on every merge to main (not just changed strings). The TMS detected "new content" because the source file hash changed (the JSON key ordering was non-deterministic in Node.js `JSON.stringify`). DeepL re-translated 180,000 strings at $0.00002/character — 22 million characters at €20/million = €4,400 ($4,100). The billing alert fired at 3:00 AM. The team had no per-language cost tracking, so they didn't notice for 2 days. The pipeline cost $4,100 for zero net new translations. | `JSON.stringify` produces non-deterministic key ordering when objects are created with dynamic keys. The CI sent the entire `en.json` file on every merge. The TMS diff'd by file hash, not by per-key content hash. The DeepL API key had no monthly spending limit configured. | **Only push changed strings, not entire locale files.** Use content-addressable keys: generate a hash of each string value (SHA-256 of the English text) as the key identifier — `"login.cta": { "value": "Sign In", "hash": "a1b2c3..." }`. Push only strings where the hash changed. Use `JSON.stringify` with sorted keys: `JSON.stringify(obj, Object.keys(obj).sort())` or use a deterministic serializer. Set a monthly spending limit on MT API keys (DeepL: "Cost Control" in account settings; Google Cloud: quota per API key). Add a CI check: if the number of strings pushed to TMS exceeds the number of changed strings by >5%, fail the pipeline and alert. |
| TMX import duplicated 50,000 segments because source language codes mismatched — TM leverage dropped from 74% to 12%, translation costs tripled for 4 months | An acquired company's TMX file used `srclang="en-US"` but the acquiring company's TMS project was configured for `srclang="en-GB"`. The TMS treated these as different languages — every segment from the acquired TM was stored under `en-US` but the project queried `en-GB`. The TM showed 0% leverage because it couldn't match `en-GB` source segments to `en-US` TM entries. The team assumed "the import failed" and re-imported the same TMX 3 times, creating 150,000 duplicated segments. When the mismatch was discovered 4 months later, the TM had to be purged and rebuilt, costing $28,000 in re-translation. | TMX files declare `srclang` in the header: `<header srclang="en-US" ...>`. TMS projects have a source language configured at project creation. Phrase, Crowdin, and Lokalise all treat `srclang` as a match key — `en-US` and `en-GB` are distinct source languages. The import script didn't validate `srclang` against the project configuration. | **Validate TMX `srclang` before import.** Extract with: `xmlstarlet sel -t -v '/tmx/header/@srclang' file.tmx`. Compare against TMS project source language — reject or auto-remap if they differ (en-US → en-GB remapping strips region). Run TMX import in dry-run mode first: upload to a staging project, count matched segments, verify >80% leverage before importing to production. Use `srcLang` as a project-level constant, not a per-file variable — all TMX files must declare the same source language code. After import, query TM statistics: total segments, unique English sources, average match percentage — if duplicate count > 10% of total, investigate before translators start. |
| Quality gate was bypassed by a race condition — the CI step marked "passed" before the ICU syntax check completed, shipping 1,200 broken strings to Russian users | The quality gate ran as a parallel CI job: `check-icu-syntax` alongside `lint` and `test`. The pipeline's "required checks" list was configured as: `lint AND test OR check-icu-syntax` — a misconfigured GitHub branch protection rule. The `OR` meant the gate passed if either `test` or `check-icu-syntax` succeeded. `test` always finished first (30s), `check-icu-syntax` took 90s. The pipeline merged the PR as soon as `test` passed, before `check-icu-syntax` could fail. Russian users received strings where `{count, plural, one {# сообщение} few {# сообщения} many {# сообщений}}` was missing the closing `}` — ICU parsers rendered the raw syntax as text. 1,200 strings displayed broken ICU placeholders for 3 days. | GitHub branch protection's "Require status checks to pass before merging" uses the check name from the CI provider. When a check is listed but the CI doesn't run it (because of `OR` logic in the workflow file), GitHub considers the requirement satisfied. The team copied a CI template without understanding the branch protection interaction. | **Quality gates must be sequential, not parallel.** Run ICU validation as a step in the main pipeline job (not a separate parallel job): `jobs: validate: steps: [lint, test, check-icu-syntax]`. If a step fails, the entire job fails. Never use `OR` in required status checks. Add a final gate step that asserts: `if: always() && steps.check-icu-syntax.outcome != 'success'` → fail build. Test the gate: create a PR with intentionally broken ICU syntax and verify the pipeline blocks merge. Run the quality gate on merge to production as well (not just PRs) — a direct push to main bypasses branch protection. |
| Glossary was ignored by TMS for MT-only workflows — the brand name "Vanta" was translated to "ванта" (Russian for "bilge water") across 12 locales | The company name "Vanta" was locked in the Phrase glossary as `translate: false` — meaning "never translate, use as-is." This worked for human translators. But the DeepL MT workflow was configured as a separate pipeline step that bypassed glossary enforcement: source strings → DeepL → TMX → TMS. DeepL translated "Vanta" phonetically to Russian (`ванта` = nautical term for bilge water), Chinese (`万塔` = "ten thousand pagodas"), and Arabic (`فانتا` = the soda brand). The glossary was only checked during human post-editing, which was skipped for "low-priority" locales. 12 locales shipped with the brand name destroyed. A customer in Moscow emailed: "Are you a cleaning company or a security company?" | The TMS glossary enforcement is applied at the TMS translation memory lookup step — but MT engines query the TMS via API without glossary context unless explicitly configured. DeepL's "Glossary" feature is a separate API endpoint (`/v2/glossaries`) that must be explicitly called before translation. The team assumed "TMS glossary = MT uses it" — they're separate systems. | **Apply glossary BEFORE MT, not after.** For DeepL: create a glossary via `POST /v2/glossaries` with all protected terms, then include `glossary_id` in every `/v2/translate` request. For Google Cloud Translation: use `glossaryConfig` in `translateText`. For Azure Translator: use custom translator with dictionary entries. Validate post-MT: run `grep -rF "ванта\|万塔\|فانتا" locale/` in CI — fail if any protected brand term appears in translated output. Add a glossary compliance check: for each locked term in the glossary, assert it appears verbatim in every locale's translation file. A brand term that doesn't match the glossary is a release blocker. |
| Translation context was lost — linguists received isolated strings like "Run" without UI screenshots, producing 450 context-critical errors across 8 locales | The developer extracted strings from a fitness app using `i18next-scanner` which emitted only the English string and a key. The string `"Run"` appeared in 3 contexts: a button label ("Start Run"), a navigation tab ("Run History"), and a status badge ("Run in Progress"). Without screenshots or developer comments, Japanese translators rendered all three as `"走る"` (the verb) — the tab label should be `"ランニング履歴"` (running history), the status should be `"実行中"` (executing). Context-blind translation caused 450 errors across 8 locales. The German translation for the status badge became `"Laufen Sie"` (imperative: "Run!"), which German users found comically aggressive. | `i18next-scanner` extracts keys and English strings, not UI context. The TMS (Lokalise) supports screenshots attached to keys, but no screenshots were uploaded because "the CI pipeline doesn't take screenshots." Translators made their best guess without seeing where the string appeared in the UI. | **Attach screenshots to every translatable key.** Upload screenshots automatically: run Playwright E2E tests that screenshot every page and map each screenshot to the keys visible on that page. Lokalise and Phrase accept screenshot uploads: `POST /api2/projects/{id}/screenshots`. For context-critical strings, add developer comments: `t('run.button', { context: 'Button label on the home screen that starts a run session. Maximum 12 characters.' })`. Context is part of the source string — strings without context should fail the CI extraction step. If a key has fewer than 20 characters AND no developer comment, flag it for manual review before sending to translators. |
