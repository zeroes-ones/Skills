...
When game development goes wrong, it goes wrong in predictable ways. Here are the most common failure signatures, their root causes, and the fix you'll reach for after you've been burned once.

| Symptom | Root Cause | Fix | Lesson |
|---------|-----------|-----|--------|
| Game runs 2x speed on 144Hz monitor — enemies move at double speed, timers count down twice as fast | `Update()` multiplied by `Time.deltaTime` but the code path is inside `FixedUpdate()` which already runs at a fixed timestep. Or movement is applied without delta time: `transform.position += Vector3.forward` | Apply delta time to every continuous value change: `transform.position += direction * speed * Time.deltaTime`. Verify with `Application.targetFrameRate = 60` for testing. Profile at 30, 60, and 144 FPS before shipping | Delta time is not automatic. Every `+=` in an update loop must be multiplied by `Time.deltaTime`. Physics should be in `FixedUpdate()`, rendering adjustments in `Update()` — mixing them creates frame-rate-dependent behavior |
| Fast-moving projectile passes through thin collider — bullet goes through walls, players report "ghost bullets" | Physics tunneling: the object moves farther in one frame than the collider thickness (speed × deltaTime > collider width). The physics engine checks discrete positions — if both are on opposite sides, no collision is registered | Use `Rigidbody.collisionDetectionMode = ContinuousDynamic` for fast projectiles. Increase physics tick rate: `Time.fixedDeltaTime = 0.005` (200Hz). For hitscan weapons, use `Physics.Raycast()` from previous position to current position | Physics engines sample positions at discrete intervals. An object moving 100m/s at 60 FPS teleports 1.67m per frame — it will never touch a 0.1m-thick wall. Continuous collision detection costs performance but is mandatory for fast-moving objects |
| Asset bundle unloads but memory isn't freed — loading screen 4 consumes more RAM than loading screen 1. After 10 level loads, out of memory | `AssetBundle.Unload(false)` keeps loaded textures in memory. Each scene load adds new textures without releasing old ones. The reference count doesn't hit zero because materials still hold texture references | Use `AssetBundle.Unload(true)` to free all associated objects. Or use `Resources.UnloadUnusedAssets()` after scene transitions. Implement Addressables with `Addressables.Release()` on scene unload. Profile with Memory Profiler to find retained assets | `Unload(false)` is the default people reach for because it's "safer" — but it guarantees a memory leak. Texture memory accumulates across every scene load until the app crashes |
| UI button click triggers game action AND UI action — character shoots AND inventory opens when player taps fire button | `EventSystem.current.IsPointerOverGameObject()` not checked before processing game input. The tap passes through the UI button to the game world behind it. Both the button's `onClick` and the game's `OnPointerDown` fire | Check `EventSystem.current.IsPointerOverGameObject()` before processing gameplay input. Use `GraphicRaycaster` on the UI Canvas. For the new Input System, set `InputSystemUIInputModule` and check `EventSystem.current.IsPointerOverGameObject(touch.fingerId)` | Unity's event system doesn't stop propagation at UI boundaries unless you explicitly check. Every input handler that isn't UI must gate on `IsPointerOverGameObject()` or it will fire through every UI panel |
