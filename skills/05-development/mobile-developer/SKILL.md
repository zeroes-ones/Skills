...
| Gotcha | Cost | Fix |
|--------|------|-----|
| Testing only on the flagship device — cold start takes 10 seconds on a $200 Android phone with 2GB RAM, 53% abandonment | $30K-$80K in poor reviews and lost users in emerging markets | Gate every release on performance metrics from a representative low-end device. Cold start <3s, scroll at 60fps, memory under budget. The flagship on your desk is not your user's phone. |
| Designing navigation without deep links — push notification taps open home screen instead of target content | $15K-$40K in broken re-engagement campaigns | Every screen reachable via URL scheme or universal link. Test deep links from cold start, warm start, and killed state. `Linking.getInitialURL()` in root `useEffect`. Deep linking is not optional for apps with notifications. |
