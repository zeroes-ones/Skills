...
## Footguns
<!-- DEEP: 10+min — war stories from production mobile apps -->

| Footgun | What Happened | Root Cause | How to Prevent |
|---------|---------------|------------|----------------|
| iOS background upload killed at 27 seconds — 1,200 users lost partially synced data, 340 had corrupted local databases | A photo backup app used `URLSession.background(withIdentifier:)` to upload 50MB batches. iOS grants ~30 seconds of background execution time (`beginBackgroundTask` expiration). At 27 seconds, iOS sent `expirationHandler` — the app called `endBackgroundTask` immediately, but the `URLSession` upload task was at 48% complete. The partial upload left the server with a truncated file and the local SQLite database with a record marked "synced" (optimistic update) for data that never arrived. Users saw their photos as "backed up" when they weren't. 340 users lost photos permanently because the corrupted local DB records were treated as source of truth. | The app used optimistic local state updates (mark-as-synced before confirmed) without a reconciliation strategy. `beginBackgroundTask` was treated as "request more time" rather than "prepare for imminent termination." The `expirationHandler` didn't roll back the optimistic update. | **Never optimistically mark data as synced until the server confirms receipt.** Use a sync queue with 3 states: `pending` → `uploading` → `confirmed`. On `expirationHandler`: cancel in-flight tasks, revert all `uploading` records to `pending`, and call `endBackgroundTask` only after the revert completes. Use `BGTaskScheduler` (iOS 13+) for long-running tasks — it grants up to 30 minutes on battery, not 30 seconds. Store upload progress per-chunk so you can resume from the last confirmed byte, not restart from zero. Test background termination: `kill(getpid(), SIGKILL)` mid-upload in a debug build and verify zero data corruption. |
| SQLite WAL file corruption from concurrent writes across 3 threads — app crashed on launch for 8,000 users after an update | A notes app used a single `SQLiteDatabase` instance shared across: (1) UI thread — auto-save every 3 seconds, (2) background sync thread — pull remote changes, (3) push notification handler — save incoming note. All three writers used the same database handle without a write-ahead log (WAL) journal mode. Under concurrent writes, SQLite's default rollback journal mode (DELETE) serializes writes — but the app didn't use `SQLiteOpenHelper.getWritableDatabase()` correctly across threads. Two writes collided, the WAL file was not checkpointed, and the database file ended with a torn page. On next launch, `sqlite3_open_v2` returned `SQLITE_CORRUPT`. 8,000 users saw a white screen on launch because the corrupt database threw an unhandled exception in the `Room` database initialization. | Android's SQLite defaults to DELETE journal mode, which serializes writes but requires correct synchronization. The app shared the database helper across threads without using a mutex. Room handles this, but the team was using raw `SQLiteDatabase` with manual threading. WAL mode was never enabled. | **Enable WAL mode immediately after database creation.** `PRAGMA journal_mode=WAL;` — WAL allows concurrent reads and writes (readers don't block writers). Use Room (Android) or GRDB/FMDB with a single serial queue (iOS) for all writes — never write from multiple threads. Set `PRAGMA busy_timeout=5000;` so concurrent writes block rather than failing. Run `PRAGMA integrity_check;` on every cold launch and trigger an automatic backup + rebuild if it fails. Test with `StrictMode.enableDeathOnSqliteLeakedConnection()` in debug builds. |
| Deep link infinite loop crashed app after 150 consecutive re-launches — user's phone battery drained from 80% to 12% in 40 minutes | The app handled deep links via `onNewIntent()` → parse URI → check auth state → navigate. A race condition existed: if the auth token was expired, the navigation redirected to the login screen — but the login screen's `onResume()` re-processed the pending deep link from `intent.data` without clearing it first. This created a loop: deep link → auth expired → login screen → onResume reads intent → deep link → auth expired... The loop executed ~40 times per second. The app crashed after ~150 iterations with `StackOverflowError` (Android) or `EXC_BAD_ACCESS` (iOS from stack exhaustion). The user's phone was unresponsive for 40 minutes. | The deep link intent was never consumed — `intent.data` remained set after the first processing attempt. The login screen's lifecycle method re-read the same intent. Android's `onNewIntent()` should call `setIntent(intent)` to update the pending intent, but this was omitted. There was no loop detection. | **Always consume deep links after processing.** Call `intent.data = null` on Android after handling, or use a flag: `boolean handled = processDeepLink(intent); if (handled) { intent.removeExtra("deep_link"); }`. On iOS: clear `connectionOptions` after `application(_:open:options:)` returns. Add loop detection: store the last 5 processed deep links and their timestamps; if the same link is processed more than 3 times in 10 seconds, break the loop and show a fallback screen. Test deep links with an expired auth token — manually revoke the token and click a deep link notification. |
| QR code scanner failed on all Android devices for 3 days after a "JPEG quality optimization" — 14,000 delivery scans failed | To reduce photo upload size, a developer changed `Bitmap.compress(Bitmap.CompressFormat.JPEG, 30)` (quality = 30, down from 92). The QR code scanner's `CameraX ImageAnalysis` pipeline converted YUV frames to JPEG before passing to the ZXing barcode detector. At quality 30, JPEG compression artifacts blurred the QR code's finder patterns (the three corner squares) beyond recognition. ZXing's `BarcodeReader` returned null for every frame — no error, just silent failure. The delivery app showed the camera viewfinder working perfectly, but no barcode was ever detected. 14,000 package deliveries failed to scan. The bug was deployed Friday evening and discovered Monday morning. | The developer treated the CameraX → ZXing pipeline as a black box: "I'm just reducing file size." JPEG compression at quality <60 introduces blocking artifacts that destroy sharp edges — QR codes depend on sharp transitions between black and white modules. The finder pattern tolerance in ZXing is ~15% deviation; JPEG quality 30 introduces 30-40% deviation. | **Never compress images before barcode/QR detection.** Feed raw YUV or NV21 frames directly to the detector — ZXing, ML Kit, and Vision natively accept YUV. Only compress AFTER successful detection, for thumbnail display. If you must compress before detection: minimum quality 85, verify against a test suite of 50 QR codes (various sizes, lighting conditions, angles). Add an integration test: capture a known QR code, run through the full pipeline, assert `result.text == expected`. Run this test in CI on each commit. Test with real devices in low light — JPEG artifacts are worse in low-contrast conditions. |
| App Store rejected 3 times for missing `NSPhotoLibraryUsageDescription` — each rejection cost 4 days and a growing backlog of 27 unshipped features | An iOS app used `react-native-image-picker` to let users upload profile photos. The library accessed `PHPhotoLibrary` which requires `NSPhotoLibraryUsageDescription` in `Info.plist`. The description was added to `Info.plist` but the app also used `expo-image-picker` in a different screen — Expo's plugin generates a separate `Info.plist` during `expo prebuild` that overwrote the custom one. The missing key wasn't caught locally because the iOS Simulator doesn't enforce privacy descriptions (simulator grants all permissions by default). Each App Store review cycle took 4 days. After the third rejection, the release was 12 days late and the marketing campaign for the photo feature had already launched. | iOS Simulator silently grants all privacy permissions — no alert, no crash, no log. The `Info.plist` was being overwritten by Expo's prebuild plugin without merging privacy keys. The team tested the permission flow in the simulator and it "just worked" because the simulator bypasses all privacy prompts. | **Always test permissions on a physical device, never only in simulator.** The simulator skips `TCC.db` (privacy database) lookups — every privacy-sensitive API silently succeeds. Add a CI check: parse the final `Info.plist` (post-prebuild/merge) and assert all required usage descriptions are present: `plutil -p Info.plist | grep -E 'NSPhotoLibraryUsageDescription|NSCameraUsageDescription|NSLocationWhenInUseUsageDescription'`. Use a pre-build script that validates privacy keys against a checklist. Test the rejection scenario: delete the app from a physical device, reinstall, and trigger every permission — verify the system dialog appears and is tappable. |
