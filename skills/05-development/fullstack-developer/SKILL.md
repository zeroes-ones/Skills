...
## Footguns
<!-- DEEP: 10+min — war stories from production fullstack systems -->

| Footgun | What Happened | Root Cause | How to Prevent |
|---------|---------------|------------|----------------|
| `NEXT_PUBLIC_STRIPE_SECRET` leaked to the client bundle — $47,000 charged fraudulently in 14 hours | A developer prefixed the Stripe secret key with `NEXT_PUBLIC_` because "the checkout page needs it to initialize Stripe." Next.js inlines all `NEXT_PUBLIC_*` environment variables at build time into the client JavaScript bundle. The secret key appeared in `/_next/static/chunks/pages/checkout-abc123.js` — viewable by anyone who opened browser DevTools. Attackers scraped it via automated bots within 2 hours. They used it to create 1,800 fraudulent $26 charges before Stripe's fraud detection flagged the account at hour 14. Stripe reversed the charges but the merchant account was permanently restricted. | `NEXT_PUBLIC_` prefix is a Next.js convention meaning "expose to client." The developer assumed "public" meant "public-facing API," not "public to anyone who views page source." No environment variable linter was configured. No secrets scanner ran in CI. | **Never prefix secrets with `NEXT_PUBLIC_`.** Server-only env vars: `STRIPE_SECRET_KEY` (no prefix) — only accessible in `getServerSideProps`, API routes, and server components. Client-safe env vars: `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`. Add a CI check: `grep -r 'NEXT_PUBLIC_.*SECRET\|NEXT_PUBLIC_.*KEY\|NEXT_PUBLIC_.*TOKEN'` and fail the build. Use `dotenv-vault` or `infisical` for secret management. Run `git-secrets` or `trufflehog` in pre-commit hooks. |
| CORS configured as `Access-Control-Allow-Origin: *` with `Access-Control-Allow-Credentials: true` — all browsers silently rejected every authenticated request | To "fix" a CORS error during development, a developer set `cors({ origin: '*', credentials: true })` in Express. Chrome and Firefox silently blocked every request because the CORS spec forbids wildcard origin with credentials — the browser drops the response without any console error visible by default. The team deployed to production and spent 4 hours debugging "the API works in curl and Postman but not in the browser." | The CORS spec (Fetch Standard §3.2.5) explicitly states: if `Access-Control-Allow-Credentials` is `true`, `Access-Control-Allow-Origin` cannot be `*`. Browsers enforce this silently — the response is a network error with no helpful error message. The developer tested with Postman (which ignores CORS) and assumed the fix was correct. | **Always mirror the request origin when credentials are needed.** `cors({ origin: req.headers.origin, credentials: true })` — and validate against an allowlist: `const allowed = ['https://app.example.com']; if (allowed.includes(req.headers.origin)) { res.set('Access-Control-Allow-Origin', req.headers.origin); }`. Never use `*` in production. Test CORS with `fetch` from the actual frontend domain — Postman and curl bypass CORS entirely. Add an E2E test that makes a cross-origin fetch and verifies it succeeds. |
| ORM lazy loading in an API handler triggered 2,400 database queries for a single page load — the endpoint timed out at 30s | A Next.js API route fetched a list of 200 orders, then in the JSX map, accessed `order.customer.name`, `order.items[0].product.name`, and `order.shipping.address.city`. Prisma's default behavior returned only the `order` table columns — each relation access triggered a separate `SELECT` query. The result: 200 orders × (1 customer + 1 items + 1 shipping) = 600 relations × average 4 queries per nested access = 2,400 queries. The database connection pool (20 connections) saturated. All other API routes queued behind it. The page loaded in 3.2s of actual query time, but connection wait time pushed it to 30s timeout. | The developer used Prisma's default query (`prisma.order.findMany()`) which returns only scalar fields — relations require explicit `include` or `select`. Accessing `order.customer` in the response triggers a separate query per order. This is invisible in development with 5 orders. | **Always use `include` for relations accessed in the response.** `prisma.order.findMany({ include: { customer: true, items: { include: { product: true } }, shipping: true } })`. Enable Prisma query logging in development: `log: ['query']` — if you see more than 5 queries per request, inspect your includes. Add a CI test: `assert(prismaQueryCount <= 5)` per endpoint. Use `prisma.$transaction` for read queries that must be consistent. For Drizzle ORM: use `.leftJoin()` and `.findMany()` with relations. For TypeORM: set `relations` in `find()` options. |
| Server-side PDF generation in an API route blocked the Node.js event loop for 18 seconds — every other request queued behind it | A "Download Invoice" API route used `puppeteer.launch()` to generate a PDF from HTML. The PDF rendering consumed 18s of synchronous CPU time (rendering a 40-page invoice with 120 product line items). Node.js is single-threaded — the event loop was blocked for the entire 18s. All concurrent requests (login, checkout, API calls) were queued. The health check endpoint stopped responding. The load balancer marked the instance as unhealthy and drained it. A single invoice download took down the entire server. | CPU-intensive work on the main thread blocks the event loop. `puppeteer` launches a child process, but rendering and serialization happen synchronously in Node.js. The developer assumed `async/await` meant "this runs in the background" — but it only means "this returns a Promise," not "this doesn't block the event loop." | **Offload CPU-intensive work to a worker or separate service.** Options: (1) Queue PDF jobs in Bull/BullMQ with a dedicated worker process on a separate Dyno/VM, (2) Use `worker_threads` for Node.js ≥ 12, (3) Use a serverless function with a 30s timeout (AWS Lambda, Vercel Edge Functions). Never generate PDFs, process images, or run ML inference in the same process that handles HTTP requests. Add a health check endpoint that measures event loop lag: `toobusy-js` middleware that returns 503 if lag > 200ms. |
| Zod validation passed on the server but failed on the client — `z.coerce.date()` parsed `"2024-13-45"` as `Invalid Date` in the browser but threw in Node.js | A shared Zod schema used `z.coerce.date()` for a birthdate field. On the server (Node.js 20), `new Date("2024-13-45")` throws a RangeError. But in the browser (Chrome V8), the same string produces `new Date("Invalid Date")` — which `z.coerce.date()` treats as... a Date object. Zod's `.date()` refinement only checks `instanceof Date`, not `!isNaN(date.getTime())`. The form submitted successfully in dev (local Node.js), failed in QA (browser validation kicked in, but the error was swallowed by the form library), and corrupted the database with `"Invalid Date"` strings when the API received them from browser submissions. | `z.coerce.date()` calls the `Date` constructor, which behaves differently across JavaScript runtimes. Node.js throws on invalid date strings in newer versions; browsers silently produce an `Invalid Date` object. Zod does not validate date validity by default — `.date()` only checks `instanceof Date`. | **Always add `.refine((d) => !isNaN(d.getTime()), 'Invalid date')` after `z.coerce.date()`.** Better: use `z.string().datetime()` or `z.string().date()` and parse with `date-fns`/`dayjs` for consistent cross-runtime behavior. Test Zod schemas in BOTH runtimes — run `vitest` with `environment: 'jsdom'` for client-side schemas and `environment: 'node'` for server-side. Add a contract test that validates the same schema against the same inputs in both environments. |
