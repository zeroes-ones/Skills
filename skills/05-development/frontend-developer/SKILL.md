...
## Footguns
<!-- DEEP: 10+min — war stories from production frontend systems -->

| Footgun | What Happened | Root Cause | How to Prevent |
|---------|---------------|------------|----------------|
| Next.js SSR memory leak from unpatched third-party analytics script — server OOM-killed every 6 hours for 3 weeks | A marketing analytics script (`<Script>` with `strategy="afterInteractive"`) loaded a 1.2MB WebAssembly module that allocated a new 4MB buffer per page view and never released it. On a 512MB DigitalOcean droplet handling 40 RPS, the Node.js heap grew from 180MB to 490MB in 6 hours. The OOM killer terminated the process. PM2 restarted it, and the cycle repeated. The team attributed crashes to "Next.js being unstable" until a heap snapshot revealed 2,400 unreleased ArrayBuffers from the analytics SDK. | The third-party script's WebAssembly module used `malloc` without `free` for transient buffers. The team added the script without auditing its memory behavior — the marketing team requested it, engineering dropped it in. No memory profiling was done in staging. PM2 auto-restart masked the root cause. | **Profile third-party scripts under load before deploying.** Run `node --inspect` with Chrome DevTools Memory profiler, record a heap snapshot at 0 RPS and at 500 RPS after 1 hour. Compare ArrayBuffer, String, and Object counts. If any third-party script leaks >10MB/hour, isolate it in a Web Worker or load it via Partytown. Set a PM2 `max_memory_restart` of `400M` as a safety net, but treat every restart as a P2 incident requiring root cause analysis. |
| Initial JS bundle ballooned from 148KB to 2.1MB gzipped after a single import — TTI went from 2.1s to 11.6s on Moto G4 | A developer imported `format` from `date-fns` using a barrel import: `import { format } from 'date-fns'`. The barrel file re-exports all 300+ functions, and tree-shaking failed because date-fns uses CommonJS interop that bundlers can't statically analyze. The entire 2.4MB library landed in the client bundle. First Contentful Paint on a throttled Moto G4 went from 2.1s to 7.8s, Time to Interactive from 3.4s to 11.6s. Mobile conversion dropped 18% in 4 days before the regression was caught by a Lighthouse CI alert. | Barrel imports defeat tree-shaking when the library doesn't use ESM with `sideEffects: false` in package.json. date-fns v2.x uses CommonJS modules with a barrel index — `import { format } from 'date-fns'` imports every function because the bundler can't determine which exports are used without static ESM. | **Always deep-import from utility libraries.** `import format from 'date-fns/format'` or use date-fns v3+ with ESM. Add `no-barrel-import` ESLint rule (`import/no-extraneous-dependencies` with path check). Run `@next/bundle-analyzer` in CI on every PR and set a bundle size budget: initial JS < 150KB gzipped per route. Any PR that increases bundle by >10% requires explicit approval. |
| Search autocomplete fired 47 stale requests simultaneously — UI displayed wrong results for 12 seconds as slow network responses arrived out of order | A product search autocomplete debounced at 300ms, but network latency varied from 80ms to 12s on 3G connections. The component stored the latest response in state without request sequencing. A user typed "winter jacket" → 5 keystrokes → 5 requests. The "wi" request (1.2MB of results) returned after 12s and overwrote the "winter jacket" results (80ms), showing irrelevant items. The user saw results flip back to wrong data 12 seconds after typing. | The autocomplete updated state with whichever Promise resolved last, not the most recently initiated request. `fetch('/api/search?q=' + query)` inside `useEffect` has no way to cancel or sequence requests — React's state batching doesn't solve the ordering problem. | **Use AbortController + request sequencing.** `useEffect(() => { const controller = new AbortController(); fetch(url, { signal: controller.signal }).then(setResults); return () => controller.abort(); }, [query])`. For frameworks: TanStack Query's `queryKey: ['search', query]` automatically deduplicates and sequences. SWR's `useSWR` with key-based caching handles this. Always test with Chrome DevTools Network throttling — add a "Slow 3G" profile and verify the last-typed query always wins. |
| CLS of 0.42 on mobile — ad slots injected via GPT (Google Publisher Tags) without reserved space shifted layout 340px | A news site loaded Google Ad Manager ads asynchronously via `googletag.display()`. The ad container had no explicit dimensions — its height was `auto` until the ad loaded (300ms-2s later). When a 250px-tall banner loaded, it pushed the article text down by 340px (250px ad + 90px margin collapse). Users tapped the wrong article link because the entire page shifted under their finger. Bounce rate increased 27%. Revenue dropped because users left before seeing the ad. | GPT's async rendering inserts the ad iframe into a container with no reserved height. The layout engine reflows everything below the container. The ad team treated the container as a "slot" (empty until filled) rather than a "placeholder" (space reserved). | **Reserve space for every dynamic content injection.** For ads: set `min-height` matching the smallest expected creative (e.g., `min-height: 250px` for a medium rectangle). For images: always use explicit `width`/`height` or `aspect-ratio`. For fonts: use `font-display: optional` + `size-adjust` in `@font-face` to prevent FOIT/FOUT layout shift. Add a CLS budget in Lighthouse CI: 0.1 on mobile. Test with `--throttling.cpuSlowdownMultiplier=4` to simulate mid-range devices. |
| CSS-in-JS runtime overhead caused 480ms INP on product listing page — every keystroke in the filter input re-computed 1,800 style objects | An e-commerce product grid used Emotion's `css` prop on every `<ProductCard>`. The filter input's `onChange` triggered a React re-render of all 200 visible cards. Each card had 6 `css` prop calls, generating 1,200 style objects per render (200 × 6). With React 18's automatic batching, this happened synchronously during the input event. The 477ms task blocked the main thread — characters appeared 400ms after typing on a Pixel 5. INP (Interaction to Next Paint) was 480ms, well above the 200ms "good" threshold. Mobile filter usage dropped 41%. | `css` prop and `styled()` are runtime APIs — they generate new style objects on every render. With 200 items × 6 style calls, the style computation alone consumed 320ms of the 477ms task. The team treated CSS-in-JS as "free" because it felt ergonomic during development with 5 items. | **Move styles to compile time.** Use zero-runtime CSS solutions: Tailwind (purges to static CSS), Vanilla Extract (TypeScript → CSS at build time), Pigment CSS (MUI's zero-runtime), or CSS Modules. If you must use runtime CSS-in-JS, memoize styled components: `const StyledCard = styled(Card)(...)` outside the render function — never create styled components inside render. Profile with React DevTools Profiler: any component re-rendering >1ms with CSS-in-JS overhead is a candidate for extraction. |
