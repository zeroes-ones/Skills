...
- **Deploying to mainnet without testnet simulation under realistic network conditions** — the contract passes all tests on a local Hardhat node with 0 gas constraints and instant block times. On mainnet, an NFT mint causes gas to spike to 500 gwei for 20 minutes. Your users' transactions are stuck pending for 45 minutes, then revert — but they still paid gas fees for the failed transactions. 2,000 users lose an average of $25 each in wasted gas, and your protocol gets "#rugpull" trending on Crypto Twitter. **Total cost: $50K-$200K in user gas losses and permanent protocol reputational damage from a launch that wasn't tested under network congestion — users who lose money to gas on your contract will never interact with it again.** Fix: simulate mainnet conditions on testnet — use Forge's `--gas-price` flag to test at 200/500/1000 gwei. Test with mainnet-forked state from a high-congestion block (e.g., during a popular NFT mint). Set conservative gas limits; never use `gasleft()` for logic. Include a gas-estimation buffer in your UI that warns users and suggests timing.
- **Using `block.timestamp` or `blockhash` for randomness in games and lotteries** — `block.timestamp` can be manipulated by miners/validators within a ~15-second window. `blockhash` is known to the miner before they publish the block. A lottery contract that uses `uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty)))` as its random source is trivially exploitable: a miner simulates the outcome before committing the block and only includes the transaction if it wins. Over 1,000 lottery rounds with an average pot of $500, the miner extracts 100% of the value. **Total cost: $100K-$500K extracted by miners/validators from contracts using predictable on-chain randomness — the money is gone, unrecoverable, and the contract is permanently gameable until upgraded.** Fix: use Chainlink VRF (Verifiable Random Function) for provably fair randomness. For lower-stakes applications, use a commit-reveal scheme where users commit a hash of their random input before the outcome-determining block, then reveal it after. Never use any single block parameter (`block.timestamp`, `blockhash`, `block.difficulty`, `block.number`) as the sole entropy source.
...
When smart contracts go wrong, they go wrong in predictable ways. Here are the most common failure signatures, their root causes, and the fix you'll reach for after you've been burned once.

| Symptom | Root Cause | Fix | Lesson |
|---------|-----------|-----|--------|
| Transaction reverts with `arithmetic underflow/overflow` — contract worked for months, then suddenly all transfers fail | Pre-Solidity 0.8 arithmetic wraps around silently. A token balance calculation overflows `uint256` and wraps to a near-zero value. Users who should have millions of tokens see zero balances | Upgrade to Solidity ≥0.8.0 which includes built-in overflow checks. For existing contracts, use OpenZeppelin's `SafeMath` library on every arithmetic operation: `a.add(b)` not `a + b`. Add fuzz tests with `bound()` on all arithmetic paths | Overflows don't fail — they produce wrong results silently. A $100M protocol can be drained by a single unchecked addition. Solidity 0.8.0's checked arithmetic is the cheapest insurance in crypto |
| Storage collision after proxy upgrade — contract state is garbled, user balances scrambled, funds inaccessible | Upgradeable proxy patterns (UUPS/Transparent) store state in the proxy contract. If the new implementation adds storage variables at the beginning or middle of the struct, all subsequent storage slots shift — reading `balances` returns `owner` data | Append new storage variables at the end of the contract. Never remove or reorder existing variables. Use `@custom:storage-location erc7201:namespace` (ERC-7201) for namespaced storage. Run storage layout diff in CI before every upgrade | Proxy storage layout is append-only. A single reordered variable corrupts every state variable below it. The compiler won't warn you — your tests will pass because they deploy fresh, not upgraded |
| Sandwich attack drains every swap on DEX — users consistently receive worse prices than quoted, MEV bots extract millions | Transaction ordering in the mempool is public. Bots see your swap, front-run it to move the price, let your transaction execute at the worse price, then back-run to profit. Every trade through a public mempool is a potential victim | Use `slippage` protection: `amountOutMin = expectedAmount * 0.995` (0.5% tolerance). Submit transactions through Flashbots/MEV-Protect to bypass the public mempool. Use off-chain order matching with on-chain settlement for large orders | The mempool is a public bulletin board. Every unconfirmed transaction is visible to every MEV searcher. Slippage protection isn't a UX nicety — it's the difference between a trade and a donation to bots |
| `transfer()` returns `false` but transaction succeeds — tokens leave user's wallet, contract state says they never arrived | Some ERC-20 tokens (notably USDT) return `false` on failed transfers instead of reverting. The calling contract checks nothing, assumes success, and credits the user with tokens they never received | Always use OpenZeppelin's `SafeERC20` library: `token.safeTransfer(to, amount)` which reverts on failure. Never call `IERC20(token).transfer()` raw. Audit every token interaction path for unchecked return values | Not all ERC-20s follow the spec. USDT's non-standard return behavior has caused more DeFi bugs than any smart contract vulnerability. SafeERC20 is not optional |
| Oracle reports stale price — protocol liquidates healthy positions or allows borrowing against worthless collateral | Chainlink oracle stops updating during high gas or network congestion. `latestRoundData()` returns the last known price which may be hours old. Lending protocol uses stale price to calculate collateral ratios | Always check `answeredInRound > 0` and `updatedAt > block.timestamp - stalenessThreshold` (e.g., 3600 for 1-hour heartbeat). Use fallback oracle (e.g., TWAP from Uniswap with 30-min window). Circuit-break: pause protocol if price is older than heartbeat + grace period | Oracle failure is a correlated failure — when one price feed stalls, all protocols using it are vulnerable simultaneously. Staleness checks are one `require` statement that prevents nine-figure liquidations |
...
| Gotcha | Cost | Fix |
|--------|------|-----|
| Using `tx.origin` instead of `msg.sender` for authorization — phishing contract drains all funds | $100K-$200M in stolen assets | Always use `msg.sender` for authorization. `tx.origin` traces to the original EOA, not the immediate caller. A phishing contract tricking a user into calling it inherits `tx.origin == victim`. |
| Calling `IERC20(token).transfer()` raw instead of `SafeERC20` — USDT silently returns false, contract assumes success | $50K-$500K in locked funds | Use OpenZeppelin's `safeTransfer()` on every token interaction. USDT, BNB, and other legacy tokens don't revert on failure. One unchecked return value locks user funds permanently. |
| Using DEX spot price as oracle without staleness check — flash loan drains protocol in one atomic transaction | $1M-$100M in protocol TVL | Use Chainlink `latestRoundData()` with `updatedAt` staleness threshold (≤1 hour) plus TWAP fallback (≥30min window). Circuit-break the protocol if price is older than heartbeat + grace period. |
| Reordering storage variables in upgradeable contract — all storage slots shift, balances return garbage | $50K-$5M in corrupted state | Append new variables at the end. Never remove or reorder. Use ERC-7201 namespaced storage. Run storage layout diff in CI before every upgrade. |
