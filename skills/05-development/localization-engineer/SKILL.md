...
## Footguns
<!-- DEEP: 10+min — war stories from production localization systems -->

| Footgun | What Happened | Root Cause | How to Prevent |
|---------|---------------|------------|----------------|
| CLDR data version mismatch between server and client — Polish users saw English dates for 6 months because `formatjs` on server used CLDR 44 while client bundle had CLDR 36 | The server used `@formatjs/intl` v3.1 (CLDR 44) deployed via a Docker image updated weekly. The React client bundled `react-intl` v5.25 (CLDR 36) that shipped with the app 18 months ago and was never updated because "it worked when we shipped it." When the server added a new Polish locale date format (`d MMMM yyyy "r."` — CLDR 44 changed Polish month formatting), the client's CLDR 36 fell back to English because it didn't recognize the new format string. Polish users on the SPA saw English dates in 40% of components (server-rendered dates looked fine, client-hydrated dates were broken). | CLDR data is versioned independently from i18n libraries. `react-intl` bundles CLDR data at build time; `@formatjs/intl` pulls it at runtime from `node_modules`. The mismatch went undetected because the team tested only in English — Polish QA was done on the deployed build, not the client bundle in isolation. | **Pin CLDR versions across the entire stack.** Use a single `@formatjs/intl` version in both server `package.json` and client `package.json` — lock them with a workspace constraint: `"@formatjs/intl": "3.1.0"`. Add a CI check: extract the CLDR version from `node_modules/@formatjs/intl/package.json` on both client and server builds, assert they match. Test locale-specific formatting in both SSR and CSR modes — a Playwright test that asserts `page.locator('[data-testid="date"]').textContent()` matches the expected Polish format after hydration. |
| GeoIP-first locale detection caused GDPR fines in France and Belgium — users were redirected to French against their explicit browser preference | The app used Cloudflare's `CF-IPCountry` header to detect locale: French users got `fr-FR`, Belgian users got `fr-BE`, Canadian users got `fr-CA`. The detection ran before the user's `Accept-Language: en-US` header was checked. French-speaking users in Brussels who set their browser to English were forced to the French site. A Belgian user filed a GDPR complaint citing "right to access services in the official language of their choice." The DPA fined the company €24,000 and required a fix within 30 days because the Accept-Language header is an explicit user preference that must be respected — GeoIP is a fallback, not a primary signal. | The locale detection middleware checked GeoIP first because "it's faster" (no header parsing). The team didn't realize that under GDPR Article 8 and the EU's language rights framework, explicit user preferences (Accept-Language, stored preference) must take precedence over inferred signals (GeoIP, IP geolocation). | **Locale detection precedence must be: (1) User's explicit stored preference, (2) `Accept-Language` header, (3) GeoIP fallback — never GeoIP first.** Implement: `detector.detect(req) => storedPreference ?? negotiateAcceptLanguage(req.headers['accept-language']) ?? geoIPFallback(req.cf.country)`. Log a warning when GeoIP and Accept-Language disagree — this is a signal that your detection is overriding user intent. Respect `Accept-Language` quality values (`en;q=0.8, fr;q=0.6`). Add a language switcher visible on every page so users can override at any time. |
| ICU MessageFormat syntax destroyed by string concatenation in a React component — `{count, plural, one {# message} other {# messages}}` became the literal string displayed to users | A developer built a notification string like: `t('notifications.you_have') + ' ' + unreadCount + ' ' + t('notifications.unread_messages')`. The French translator entered the key as `'Vous avez {count, plural, one {# message non lu} other {# messages non lus}}'`. At runtime, the concatenation produced `"Vous avez {count, plural, one {# message non lu} other {# messages non lus}} 5 messages non lus"` — i18next never saw the `{count}` variable because the string was built outside the `t()` function call. The ICU syntax was rendered as literal text to 200,000 French users for 3 weeks. | String concatenation with `t()` calls destroys variable interpolation. The developer assumed each `t()` call was independent and the concatenated result would be "combined" — but ICU syntax requires the entire message (including the variable) to be inside a single `t()` call. | **Never concatenate translated strings.** Every variable-dependent message must be a single `t()` call: `t('notifications.you_have_unread', { count: unreadCount })` with the full ICU string `'{count, plural, one {# message non lu} other {# messages non lus}}'`. Add an ESLint rule that blocks `t() + t()` and string template literals mixing `t()` with variables: `eslint-plugin-i18next` rule `no-literal-string` with `mode: 'jsx-text-only'`. Test every pluralized string with count values 0, 1, 2, 5, 21 in CI — generate screenshots for each count in each locale. |
| Pseudo-localization passed CI but detected zero bugs for 8 months — the locale fallback chain was so aggressive that every missing key silently fell back to English | The CI pipeline ran `i18next-pseudo` to generate `en-XA` (pseudo-locale with accented characters, brackets, and 30% text expansion). The pseudo-locale build passed because the fallback chain was: `['en-XA', 'en-US']`. Every time a key was missing in the translation file, i18next silently used the English fallback instead of showing the pseudo-localized string. The QA team tested the pseudo-build and saw "all text is accented" (what WAS translated looked correct), but 340 keys — 12% of the app — were never translated and fell back to clean English. The pseudo-locale passed because the fallback masked the gaps. | The pseudo-locale's purpose is to expose missing translations, but a fallback chain that includes the source language defeats this. `i18next` defaults to `fallbackLng: 'en'`, which means any missing key renders in English — the exact behavior pseudo-localization was supposed to detect. | **Break the fallback chain in pseudo-locale builds.** Configure `fallbackLng: false` (or `fallbackLng: 'cimode'` for ICU debugging) when building pseudo-locale. This forces every untranslated key to display as the key name (e.g., `home.hero.title`) — immediately visible. Run the pseudo-locale build on every PR with automated screenshot comparison: any screenshot containing English text in a pseudo-build is a failed check. Count `t()` calls with missing keys via `i18next`'s `missingKeyHandler` and fail CI if count > 0. |
| User-entered LTR phone numbers inside RTL Arabic text corrupted the display — `+1-555-0123` rendered as `3210-555-1+` to 300,000 Arabic users | Arabic users entered their phone number `+1-555-0123` in a profile form. The React component rendered it inside `<p dir="rtl">{user.bio}</p>`. The browser's Unicode Bidirectional Algorithm (UBA) treated the sequence as: `+1-` (LTR-neutral at RTL context start) → `555-0123` (LTR) — but the overall paragraph direction was RTL, so the entire string was reversed. Arabic users saw their phone number backwards and credit card numbers appeared with digits reversed. Customer support received 900 complaints in 3 days. | The UBA determines text direction per-character based on Unicode properties. In an RTL container, neutral characters (like `+` and `-`) inherit the parent direction. When the parent is RTL, LTR sequences reorder. The phone number wasn't wrapped in a directional isolate so it was subject to the parent's `dir="rtl"`. | **Wrap user-entered LTR content in directional isolates.** Use `<bdi>` (bidirectional isolate) HTML element or `dir="auto"` on any container holding user-generated content. For inline content: `\u2068` (First Strong Isolate) and `\u2069` (Pop Directional Isolate) characters: `'\u2068' + phoneNumber + '\u2069'`. CSS: `unicode-bidi: isolate; direction: ltr` on any element displaying LTR data in an RTL page. Test with the string `"משה Cohen 123-4567 قال"` — it contains Hebrew, Latin, numbers, and Arabic in one string. If any segment renders in the wrong direction, your BIDI handling is broken. |
