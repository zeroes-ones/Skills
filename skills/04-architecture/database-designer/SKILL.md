...
- **No connection pooling in production.** Every application instance opens 50-200 direct database connections without a pooler (PgBouncer, RDS Proxy). PostgreSQL forks a process per connection — 1,000 connections from 10 app instances consume 1-2GB of RAM and context-switch the CPU to death, while a pooler would handle the same workload with 20-50 connections. **Total cost: $10K-$100K in unnecessary database tier scaling (doubling or tripling instance size) to handle connection overhead instead of actual query work.** Fix: place PgBouncer (transaction mode) or RDS Proxy between every application and the database. Configure pool size to `(core_count * 2)` for transactional workloads.

- **Using auto-increment IDs as external identifiers.** Teams expose `SERIAL`/`AUTO_INCREMENT` primary keys in URLs (`/orders/104729`), API responses, and webhook payloads — making database internals into permanent business identifiers. When you migrate databases, shard, merge with an acquisition, or replicate cross-region, those IDs collide or must be remapped, breaking every external reference, bookmark, and integration. **Total cost: $100K-$500K in data migration projects lasting 3-12 months, broken third-party integrations, and lost SEO rankings from URL changes on public-facing pages.** Fix: use UUIDv7 or ULIDs as the external-facing identifier from day one. Keep auto-increment for internal-only tables if needed, but never leak them past the API boundary. Add a separate `public_id` column generated at row creation time.

- **Running schema migrations that lock large tables during business hours.** `ALTER TABLE orders ADD COLUMN status TEXT DEFAULT 'pending'` on a 500M-row table acquires an `ACCESS EXCLUSIVE` lock, blocking all reads and writes for the duration — which can be 30 minutes to several hours depending on row count and I/O throughput. The same migration that took 3 seconds in staging with 10K rows takes down production. **Total cost: $50K-$500K in downtime (at $10K-$100K/hour for revenue-generating systems) from a single migration that blocks transactions, queues orders, and triggers incident-response mobilization.** Fix: use zero-downtime migration strategies: add nullable columns without defaults first, backfill in batches with `UPDATE ... LIMIT`, then add the `NOT NULL` constraint and default. Use `gh-ost` or `pt-online-schema-change` for MySQL, `pgroll` or `reshadow` for PostgreSQL.
...
|---------|-----------|-----|--------|
| `SELECT * FROM orders WHERE user_id = ?` goes from 2ms to 800ms after 6 months | Missing composite index on `(user_id, created_at DESC)` — database does full table scan as table grows past 100K rows | `CREATE INDEX idx_orders_user_created ON orders(user_id, created_at DESC)` — composite index covers both the WHERE filter and the ORDER BY | Indexes that work on 10K rows fail silently at 100K. Always test query plans with production-sized datasets before shipping. |
| `deadlock detected` in PostgreSQL under concurrent checkout — two transactions updating `inventory` and `orders` in opposite order | Transactions acquire locks in different order: TX1 locks `inventory` then `orders`, TX2 locks `orders` then `inventory`. Classic circular wait | Enforce consistent lock ordering: always lock `orders` before `inventory` in every code path. Or use `SELECT ... FOR UPDATE NOWAIT` to fail fast instead of deadlocking | Deadlocks are deterministic, not random. Two transactions accessing the same two tables in opposite order will deadlock every time under enough concurrency. Consistent lock order is a code review rule, not a DBA task. |
| Migration runs fine in staging, fails in production with `table locked for too long` — blocking all writes for 45 minutes | `ALTER TABLE ... ADD COLUMN` acquires `ACCESS EXCLUSIVE` lock on PostgreSQL. Production table has 50M rows and 500 concurrent writes; staging has 10K rows and no traffic | Use zero-downtime migration pattern: (1) add column nullable with default NULL (instant in PG11+), (2) backfill in batches, (3) add NOT NULL constraint with `NOT VALID` then validate separately, (4) set default | Every DDL statement has a lock level. `ACCESS EXCLUSIVE` on a busy table means an outage. Production migrations need a lock-time budget — if it exceeds 2 seconds, use the multi-step pattern. |
| `WHERE status = 'active'` returns 0 rows but `WHERE status = 'Active'` returns 500 — case-sensitivity silently corrupting reports | Application inserted `'Active'` via one code path and `'active'` via another. No CHECK constraint or enum to enforce values | Use a lookup table (`order_statuses`) with FK constraint. Or PostgreSQL enum type. Or at minimum `CHECK (status = lower(status))`. Never rely on application-layer consistency for enumerations | Strings are not enums. If a column has a finite set of valid values, enforce it at the database level. Application code changes; constraints don't. |
| Replication lag spikes to 30 seconds during nightly batch job — read replicas serving stale data to users | Batch job does 200K `UPDATE` statements in a single transaction. Every row change writes to WAL, and replicas must replay the entire transaction before showing results | Break batch into 1000-row transactions. Or use `COPY`/`INSERT INTO ... SELECT` instead of row-by-row updates. Set `max_standby_streaming_delay` and monitor replication lag with `pg_stat_replication` | Replication lag is a function of write volume per transaction, not total writes. One 200K-row transaction blocks replicas for minutes. One thousand 200-row transactions don't. |
