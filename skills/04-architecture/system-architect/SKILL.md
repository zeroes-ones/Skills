...
|---------|-----------|-----|--------|
| System works at 100 QPS, collapses at 500 QPS with no graceful degradation | Synchronous call chain: API → Service A → Service B → Database — one slow query blocks the entire chain. Thread pool exhaustion cascades: all threads waiting on DB, no threads available to serve health checks | Add circuit breakers (Resilience4j/Polly) with fallback responses. Implement bulkheads with separate thread pools per downstream dependency. Add health checks on a dedicated thread pool — never blocked by business logic | Every synchronous dependency is a single point of failure. Circuit breakers aren't optional at scale — they're the difference between a slow component and a full outage. |
| Deploy succeeds, canary passes, 10 minutes later all users get 500 errors — rollback takes 20 minutes | Database migration was in the deploy but rollback didn't include migration reversal. New code writes to new column, old code doesn't know about it. Canary only ran for 2 minutes — not enough to hit the bad code path | Decouple deploys from migrations: (1) deploy code that works with both old and new schema, (2) run migration, (3) deploy code that uses new schema. Each step separately reversible. Extend canary to 10% traffic for 15 minutes minimum | Deploy and migration in the same step is a one-way door. If you can't roll back the migration independently from the code, you can't roll back safely. Two-phase deploys are the price of reversible releases. |
| Cache hit rate 99.8% — then 0% after Redis restart, database melts under 50x load | No cache warming strategy. Application assumes cache is warm. Redis restart flushes all keys. First request to every endpoint is a cache miss, all concurrent misses hit the database simultaneously | Implement cache warming on startup: replay top 1000 queries from access log. Use stale-while-revalidate pattern: serve stale cached data while asynchronously refreshing. Never let a cold cache produce a thundering herd | Cache hit rate doesn't matter if you can't survive a cache miss storm. Design for the cold start — every cache has one eventually. Thundering herd protection is more important than eviction policy. |
| P99 latency is 45ms on dashboard but customers report 3-second page loads — metrics are lying | P99 measured at load balancer. But page load requires: DNS (200ms) → TLS handshake (150ms) → CDN (100ms) → API (45ms) → Database (30ms) → Render (800ms). Each measured independently, user experiences the sum | Measure end-to-end latency from the user's perspective: RUM (Real User Monitoring) with Navigation Timing API. Set SLOs on the user-perceived metric, not the component metric. Alert when P75 user latency exceeds 2 seconds | Your dashboard shows component health. Your users experience system health. The gap between them is where most production incidents live. Instrument the user journey, not just the service mesh. |
| Kubernetes HPA scales pods from 3 to 30 in 90 seconds — but 12 of them crash with `connection refused` to database | Pods scale faster than database connection pool can handle. Each pod opens 20 connections. 30 pods × 20 = 600 connections. Database max_connections = 200. New pods get connection errors | Set pod connection pool to `(max_db_connections * 0.8) / max_pods`. Add connection pool metrics to HPA: if pool utilization > 80%, block scale-up until existing pods drain. Configure `maxSurge` to limit simultaneous new pods | Autoscaling without connection pool awareness is a self-DOS. Every new pod that can't connect to the database is a pod that's serving 500s and counting toward your error budget. |
