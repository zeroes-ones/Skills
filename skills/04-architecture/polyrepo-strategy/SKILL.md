...
|---------|-----------|-----|--------|
| "Works on my machine" — CI passes on PR but `main` build fails after merge | Repo A publishes new library version v2.3.0 with breaking change. Repo B pins to `^2.0.0` in `package.json`. CI cache had v2.2.0. After merge, CI resolves to v2.3.0 fresh and breaks | Pin exact versions (`2.2.0` not `^2.0.0`) for cross-repo dependencies. Run integration build that checks out all repos at HEAD and builds together. Require PRs to pass against latest of all dependencies, not just cached | Semver ranges in polyrepos are time bombs. `^2.0.0` means "anything under 3.0.0" — including versions not yet published. Pin exactly and upgrade intentionally. |
| Repo A changes its API contract, 3 downstream repos break silently — discovered in production | No contract tests between repos. Repo A removed a field from JSON response. Repo B, C, D compiled fine (they use the field at runtime, not compile time) but failed in production | Add contract tests in CI: Repo A publishes OpenAPI spec. Repos B/C/D run `pact` consumer tests against A's spec. Pipeline blocks A's merge if it would break any consumer. Consumer tests run on every dependency PR | If a change in Repo A can break Repo B in production without a compile error, you don't have an architecture — you have hope. Contract tests are the compile-time safety net for polyrepos. |
| Same shared library has 4 different versions across 8 repos — bug fixed in v1.3.0 is still live in 3 repos stuck on v1.1.0 | Library maintainer publishes fixes, but downstream repos never upgrade. No visibility into which repos use which version. No automated upgrade mechanism | Build a dependency dashboard showing version matrix across all repos. Use Renovate/Dependabot with auto-merge for patch versions. Set SLAs: repos must be within 2 minor versions of latest. Flag repos >30 days behind | Shared code that isn't consistently versioned isn't shared — it's forked. Without automation, every repo will drift to a different version, and you'll fix the same bug 8 times. |
| Feature requires changes in 4 repos — 3 merge, 1 gets stuck in review for 2 weeks. Feature is half-deployed and broken | Cross-repo feature orchestration has no atomicity. No feature flag to toggle the new behavior. Half the system expects new contract, half expects old | Use feature flags gated by a central config service (LaunchDarkly, in-house). All repos deploy with flag OFF. When all 4 repos are deployed, flip flag ON in one operation. If issues, flip OFF — instant rollback across all repos | Cross-repo features need a distributed feature flag, not a distributed merge. Deploy each repo independently, control the feature with one toggle. Coordinated merges are a scheduling problem; feature flags are an architecture solution. |
| Docker image build in Repo A pulls `lib-auth@latest` from internal registry — gets version that doesn't exist in any git tag | Internal registry allows overwriting `latest` tag. CI pipeline in lib-auth publishes new version to `latest`, overwriting previous. No immutable tags. Reproducibility is impossible | Never use `latest` tag. Every build publishes with git SHA tag (`lib-auth:abc1234`). Consumer repos reference by SHA, not version number. Registry enforces immutable tags — reject overwrites | `latest` means "whatever was built most recently." In a polyrepo, that's a different thing at 10:01 AM than it was at 10:00 AM. SHA-tagged images are the only reproducible deployment artifact. |
