#!/usr/bin/env bash
# Verification harness for library-linkage-architect
#
...
echo "=== Verifying library-linkage-architect ==="
echo ""
...
check "name matches directory" grep -q '^name: library-linkage-architect$' "$S"
...
check "keeps the three axes separate" grep -qi 'three axes\|resolution time\|unit form\|process presence' "$S"
check "asserts explicit form, not inherited default (R1)" grep -qi 'inherited' "$S"
check "asserts update model named (R2)" grep -qi 'update model' "$S"
check "asserts exposure window measured" grep -qi 'exposure window' "$S"
check "asserts ABI policy declared (R3)" grep -qi 'abi policy' "$S"
check "asserts silent ABI break (struct field / virtual method)" grep -qi 'struct\|virtual method' "$S"
check "asserts opaque types or reserved space" grep -qi 'opaque\|reserved space' "$S"
check "asserts visibility hidden-by-default (R5)" grep -qi 'hidden' "$S"
check "asserts collision test" grep -qi 'collision' "$S"
check "asserts unload reality (R4)" grep -qi 'unload' "$S"
check "asserts dlclose is not reliable" grep -qi 'unload is not\|not generally\|is not a mechanism\|not dependable\|cannot be relied' "$S"
check "asserts platform constraint check (R6)" grep -qi 'platform' "$S"
check "states static vs dynamic is not a speed win" grep -qi 'not reliably\|is not a speed\|trade, not' "$S"
check "cites the Apple size/launch trade" grep -qi 'executable' "$S"
check "names soname or loader resolution" grep -qi 'soname' "$S"
check "asserts binding mode choice (eager/lazy)" grep -qi 'binding mode\|eager\|lazy' "$S"
check "asserts fork safety" grep -qi 'fork' "$S"
check "asserts symbol prefixing" grep -qi 'prefix' "$S"
check "asserts remediation rehearsal" grep -qi 'rehears' "$S"
check "names a plugin ABI option (C / WASM)" grep -qi 'plugin abi\|wasm' "$S"
check "asserts version probe before use" grep -qi 'version probe\|abi_version' "$S"
