#!/usr/bin/env bash
# Verification harness for token-efficiency
# Run: bash scripts/verify-skill.sh
set -euo pipefail

echo "=== Verifying token-efficiency ==="
echo ""
...
# Script sanity: token-cost-calculator.py exists and runs --help
check "token-cost-calculator.py exists" test -f "$SKILL_DIR/scripts/token-cost-calculator.py"
check "token-cost-calculator.py --help runs" python3 "$SKILL_DIR/scripts/token-cost-calculator.py" --help >/dev/null 2>&1
