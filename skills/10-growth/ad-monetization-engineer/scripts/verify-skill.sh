#!/usr/bin/env bash
# verify-skill.sh — Verification script for ad-monetization-engineer skill
...
echo "=== Ad Monetization Engineer — Skill Verification ==="
...
for ref in ad-tech-stack.md header-bidding.md ad-unit-ux.md yield-optimization.md ad-quality-compliance.md mobile-ad-mediation.md ad-blocker-recovery.md analytics-reconciliation.md; do
...
# 6. Verify description format (flatten newlines for YAML > folding, handle extra whitespace)
...
if echo "$FLAT_DESC" | grep -q "Use when" && \
   ...
   echo "$FLAT_DESC" | grep -qE "Do\s+NOT\s+use"; then
    ...
    echo "DEBUG: FLAT_DESC snippet: $(echo "$FLAT_DESC" | grep -oE 'Do.*NOT.*use[^)]*' || echo 'NOT FOUND')"
    ERRORS=$((ERRORS + 1))
fi

# 7. Verify gotchas are dollar-quantified
