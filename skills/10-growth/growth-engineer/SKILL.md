...
chain:
  examples:
  - skills/10-growth/growth-engineer/examples/backtest
  consumes_from:
...
workflow:
  artifacts:
    inputs: [product-spec, funnel-data]
    outputs: [growth-plan]
  completion:
    criteria:
      - Hypotheses tied to funnel metrics with expected lift
      - Experiments scoped with success and guardrail metrics
      - Risks to the core experience flagged before launch
    evidence: required
...
- **Optimizing for clicks instead of revenue — the vanity metric trap.** A growth team A/B tests a new landing page that increases click-through rate by 35% (p<0.001) — a "massive win" celebrated at the company all-hands. But nobody measured downstream conversion: the new page attracts curious clickers who bounce at 92% on the pricing page. Net revenue per visitor dropped 18%, and the change was live for 6 weeks before anyone connected the dots. **Total cost: $50,000-$200,000 in lost revenue from A/B tests that optimize proxy metrics without measuring the metric that matters.** Fix: Every experiment must have a primary metric that maps to revenue or retention — never ship on CTR, page views, or signups alone; instrument the full funnel from click to revenue; if an intermediate metric improves but the north-star metric doesn't move within the confidence interval, the experiment failed.
- **Silent failure of analytics tracking during a critical launch.** The growth team launches a major referral program with 12 new custom events. Engineering implements all 12, but 3 events never fire because a single misnamed property (`referrer_user_id` vs `referral_user_id`) causes the payload to be silently rejected. The growth team spends 4 weeks optimizing the referral funnel using incomplete data, making changes that unknowingly hurt the 3 missing event paths. **Total cost: $40,000-$150,000 in misdirected optimization effort, degraded user experience on unmonitored paths, and opportunity cost of a broken growth loop.** Fix: Implement analytics QA as a CI step — every event definition must have a corresponding assertion test that validates it appears in the analytics debug endpoint within 30 seconds; add server-side event validation that rejects malformed events with an alert, not silently; run end-to-end analytics test before every major launch.
- **Running too many concurrent experiments without interaction detection.** The growth team runs 7 simultaneous A/B tests: new homepage, revised pricing, different CTA, modified onboarding, updated email subject, new testimonial placement, and trial length experiment. A user is in the treatment group for 4 of 7 tests, with unmeasured interaction effects. The team ships 3 "winning" variants each showing +5% individually, but combined in production the net effect is -2% because the homepage promises "unlimited everything" while the pricing test shows usage-based tiers. **Total cost: $60,000-$250,000 in negative interaction effects from concurrent experiments, wasted engineering on features that don't combine well, and regression after multi-variant rollout.** Fix: Limit concurrent experiments sharing the same user journey; use multi-variate testing (MVT) when you need to test interactions deliberately; run a holdout validation with ALL winners vs ALL controls to measure combined effect before shipping; track experiment overlap metrics.
