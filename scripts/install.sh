...
# creates global symlinks so every project can access every skill.
#
...
# bin). Dual-mode: every skill by default, or --solo/--grow subsets.
install -m 755 "$SKILLS_HOME/scripts/init-project.sh" "$HOME/.local/bin/skills-init"
echo -e "      ${GREEN}✓${NC} skills-init → $HOME/.local/bin/skills-init (default: all; --solo/--grow subsets)"
